...
class DownloadS3Images extends Command
{
    ...
    public function handle()
    {
        ...
        // Get distinct images based on tracking numbers
        ...
        ->whereIn('s.tracking_number', $trackingNumbers)
        ...
        foreach ($images as $key => $image) {
            // Create a new filename by appending tracking number to the original filename
            $trackingNumber = $trackingNumbers[$key];  // Adjust logic to get the correct tracking number for each image
            $extension = pathinfo($image, PATHINFO_EXTENSION);

            // Create a new file name with tracking number
            $newFileName = $trackingNumber . '_' . pathinfo($image, PATHINFO_FILENAME) . '.' . $extension;

            // Local directory where images will be saved
            $localDir = storage_path('app/public/return_note_images');
            $localPath = $localDir . '/' . $newFileName;

            // Ensure the directory exists
            if (!file_exists($localDir)) {
                mkdir($localDir, 0775, true);  // Create directory with appropriate permissions
            }
            ...
            if (Storage::disk('s3')->exists($s3Path)) {
                try {
                    ...
                    // Check if the file already exists locally, skip if it does
                    if (!file_exists($localPath)) {
                        file_put_contents($localPath, $fileContent);  // Directly use file_put_contents
                        $this->info("Downloaded: $newFileName");
                    } else {
                        $this->info("Skipping duplicate: $newFileName");
                    }
                } catch (\Exception $e) {

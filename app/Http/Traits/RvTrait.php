    ...
    protected function on_hold_for_self_collection(Request $request)
    {
        ...
        if ($shipmentId) {
            if (Shipment::where('id', $shipmentId)->where('shipper_status_id', '!=', 15)->exists()) {
                ...
                // return response()->json(['status' => 0, 'error' => 'Shipment already updated to Shipment - On Hold for Self Collection!']);
            }
        } else {
            return ['status' => 0, 'error' => "Shipment ID Not selected!"];
            // return response()->json(['status' => 0, 'error' => 'Shipment ID Not selected!']);
        }
    }
    ...
    protected function rv_shipment_assign_agent_by_admin($data)
    {
        try 
        {
            ...
            if($shipments_journey){
                $rv_shipment_assign_agent = RvShipmentAssignAgent::where('rv_state_id', 1)->where('assigned_to_type_id', 1)->where('agent_id',$data['agent_id'])->latest()->first();
                ...
                if(!$rv_shipment_assign_agent){
                    ...
                    $rv_shipment_assign_agent->assigned_to_type_id  = 1;
                    $rv_shipment_assign_agent->save();
                }

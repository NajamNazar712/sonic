        ...
        if (count($regular_pickups) > 0){
            ...
            foreach ($regular_pickups as $regular_pickup) {
                ...
                if(in_array($day, $pickup_days)){
                    ...
                    // dd($pickup_request);
                    if($pickup_request){
                        if( $date> $pickup_request->pickup_date)
                        {
                            $pickup_request_id = AddV3PickupController::add($pickup_request->shipper_id, $pickup_request->pickup_type, $pickup_request->pickup_address_id, $today, $pickup_request->city_id, $pickup_request->time_range_id, $pickup_request->pickup_shipment_type_id, $pickup_request->weight, $pickup_request->booked, $pickup_request->pieces, $pickup_request->special_request, 1, $global_admin_id, $pickup_request->walkin_name, $pickup_request->walkin_address, $pickup_request->walkin_contact, $pickup_request->segment_id, $pickup_request->sub_segment_id);

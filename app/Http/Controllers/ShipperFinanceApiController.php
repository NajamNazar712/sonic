        ...
        $retail_payments = DB::table('retail_done_payments')
            ...
            ->leftjoin('banks_lists as ub','ub.id','=','rsi.bank_id')
             ->select(
                 DB::raw("
            JSON_OBJECT(
                'delivered_tracking_numbers', GROUP_CONCAT(IF(rdps.type = 0, s.tracking_number, NULL)),
                'return_tracking_numbers', GROUP_CONCAT(IF(rdps.type = 1, s.tracking_number, NULL)),
                'adjusted_tracking_numbers', GROUP_CONCAT(IF(rdps.type = 2, s.tracking_number, NULL))
            ) as tracking_numbers
        "),

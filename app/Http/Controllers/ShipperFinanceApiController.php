    ...
    private function retail_payment_list($retail_id)
    {
        $retail_payments = DB::table('retail_done_payments')->join('retail_shipper_infos as rsi', 'retail_done_payments.user_id', '=', 'rsi.id')
            ...
            ->join('banks_lists as ub','ub.id','=','rsi.bank_id')
             ->select('retail_done_payments.id', 'rsi.id as user_id', 'rsi.shipper_name as shipper', 'c.name as city', 'rsi.shipper_phone_no', 'rsi.shipper_address', 'retail_done_payments.total_shipments', 'retail_done_payments.delivered_shipments', 'retail_done_payments.adjusted_shipments', DB::raw('SUM(rdps.amount) as total_amount') , DB::raw('SUM(rdps.payable) as total_payable'), 'retail_done_payments.ibft_charges','ub.name as bank', 'retail_done_payments.reference_number', 'retail_done_payments.created_at as done_at', 'retail_done_payments.status')

...
class ShipperReceivingSheetHistoryController extends Controller
{
    ...
    public function received_shipments(Request $request) {
      if ($request->receiving_sheet_id != 'NaN') {
        $receiving_sheet_received = ReceivingSheetReceived::where('receiving_sheet_id', $request->receiving_sheet_id)->where('user_id', Auth::id())->where('status', 0)->get();
      }
      else {
        $receiving_sheet_received = ReceivingSheetReceived::whereNull('receiving_sheet_id')->where('user_id', Auth::id())->where('pickup_address_id', $request->pickup_address_id)->where('status', 0)->get();
      }
    ...
    public function void(Request $request) {
      ...
      foreach (ReceivingSheetReceived::where('receiving_sheet_id', $request->receiving_sheet_id)->get() as $receiving_sheet_received) {
        $receiving_sheet_received->status = 1;
    ...
    public function create(Request $request) {
      ...
      foreach (ReceivingSheetReceived::whereNull('receiving_sheet_id')->where('user_id', Auth::id())->where('pickup_address_id', $request->pickup_address_id)->where('status', 0)->get() as $receiving_sheet_received) {

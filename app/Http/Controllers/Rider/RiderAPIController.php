...
//                            dispatch(new ProcessAgentCallMonitoring($arr));
                    ...
                    if (DeliveryNoteShipment::join('delivery_notes as dn', 'delivery_note_shipments.delivery_note_id', 'dn.id')->where('dn.id', $request->delivery_note_id)->where('shipment_id', $request->shipment_id)->where('dn.rider_id', $rider_id)->exists()) { {
                            ...
                            if (DeliveryNote::where('id', $request->delivery_note_id)->where('pending_status', 0)->exists()) {
                                ...
                                elseif ($shipment->booking_type_id == 2) {
                                    ...
                                    $shipment->consignee_status_id = 30;
                                    $shipment->received_amount = $shipment->amount;
                                    // dispatch(new LastMileApp($shipment->id,$request->delivery_note_id,$rider_id,$shipment->shipper_status_id,$added_at,$rider_delivery,2));
                                    ...
                                    $delivered_parcels = count($item_ids);
                                    if ($total_parcels == $delivered_parcels) {
                                        // dispatch(new LastMileApp($shipment->id,$request->delivery_note_id,$rider_id,36,$added_at,$rider_delivery,2));
            ...
            if ($payslip->exists()) {
                ...
                <html lang="en">
                  <head>
                    ...
                    <link rel="stylesheet" type="text/css" href="' . asset('rider-assets/css/bootstrap.min.css') . '">
    ...
    public function leave_index(Request $request)
    {
        ...
        if ($employee) {
            ...
            $data['riderrover_email'] = $employee->line_manager->email;
            $data['riderrover_name'] = $employee->line_manager->name;
            $data['user_type'] = 0;
            return response()->json(['status' => 0, 'data' => $data]);
        }
    ...
    public function leave_riderly(Request $request)
    {
        return response()->json(['status' => 1, 'message' => 'Please Update Your Bolt App']);
        $rules = [
            'from' => ['required'],
        ...
        if ($validate->fails()) {
            ...
            if ($employee) {
                if ($request->has('leave_id')) {
                    ...
                    NotificationsController::rider_notification(11, $rider_id, 2, $leave_request->id);
                    NotificationsController::rider_notification(12, $user->id, 1, $leave_request->id);
    ...
    public function employee_leave_list(Request $request)
    {
        ...
        $employee_leaves = EmployeeLeave::join('leave_statuses as ls', 'employee_leaves.status', '=', 'ls.id')
            ->select('employee_leaves.id as id', 'employee_leaves.from as from', 'employee_leaves.to as to', 'employee_leaves.riderlied_reason as riderlied_reason', 'employee_leaves.rejected_reason as rejected_reason', 'employee_leaves.status as status_id', 'ls.name as status')
            ...
            foreach ($dates as $date) {
                ...
                if ($attendance->exists()) {
                    ...
                    if ($attendance->leave_status == 2) {
                        $datum["status"] = 4; //adjustment riderly
                    } else if ($attendance->leave_status == 1) {
                        $datum["status"] = 5; //leave riderly
                    } else {
                        if ($shift_exists == 1) {
                            if ($attendance->clock_in_datetime) {
                                $clock_in_date = Carbon::parse($attendance->clock_in_datetime)->format("Y-m-d");
    ...
    public function check_bolt_version(Request $request)
    {
        ...
        if ($global_settings->exists()) {
            ...
            return response()->json(['status' => 0, 'rider_version' => $global_settings->setting_value]);
        } else {
            return response()->json(['status' => 0, 'rider_version' => 24]);
        }
    }
                        ...
                        foreach ($shipment_ids as $shipment_id) {
                            ...
                            if ($shipment->exists()) {
                                ...
                                if ($shipment->shipper_status_id == 1 && in_array($shipment->id, $pickup_request_shipments)) {
                                    ...
                                    $shipment->save();
                                    ShipmentsJourneyController::add($shipment->id, 53, 53, NULL, NULL, NULL, NULL, $request->pickup_request_id, $request->pickup_note_id, 1, NULL, $rider_id);
                                    ShipmentScanningJourneyController::add($shipment->id, 31, 5, $rider_id, null, null ,null,null, $request->actual_location_latitude, $request->actual_location_longitude ,'rider');
...
//                                        }
    ...
    public function check_profile_v3(Request $request)
    {
        ...
        if ($rider) {
            ...
            $rider->current_rider_version = $request->rider_version;
            $rider->save();
            $profile = Employee::where('trax_id', $rider->trax_id);
            if ($profile->exists()) {
                $profile = $profile->first();
            ...
            if ($rider->exists()) {
                ...
                if ($rider->status) {
                    ...
                    if ($rider->first_login == 0) {
                        return response()->json(['status' => 1, 'message' => "Dear " . $rider->name . "- Your request is in process and is pending for riderroval from HR."]);
                    } else {
                        return response()->json(['status' => 1, 'message' => 'Your Account is Disabled']);
                    }
                }
                ...
                if ($employee->exists()) {
                    $employee = $employee->first();
                    return response()->json(['status' => 1, 'message' => "Dear " . $employee->name . "- Your request is in process and is pending for riderroval from HR."]);
                } else {
    ...
    public function adjustment_index(Request $request)
    {
        ...
        if ($rider) {
            ...
            $data['designation'] = "Rider";
            $data['department'] = "Operations";
            $data['riderrover_email'] = $rider->line_manager->email;
            $data['riderrover_name'] = $rider->line_manager->name;
            $data['user_type'] = 0;
            return response()->json(['status' => 0, 'data' => $data]);
        }
        return response()->json(['status' => 1, 'message' => "Rider not found"]);
    ...
    public function adjustment_riderly(Request $request)
    {
        $rules = [
            'date' => ['required'],
            'reason' => ['required', 'max:500'],
        ...
        if ($validate->fails()) {
            ...
            if ($rider) {
                if ($rider->line_manager_id != null) {
                    ...
                    NotificationsController::rider_notification(17, $rider_id, 2, $leave_request->id);
                    if ($notify) {
                        NotificationsController::rider_notification(18, $leave_request->reporter_id, 1, $leave_request->id);
                    }
    ...
    public function employee_adjustment_list(Request $request)
    {
        ...
        $employee_leaves = EmployeeAttendanceAdjustment::join('leave_statuses as ls', 'employee_attendance_adjustments.status', '=', 'ls.id')
            ->select('employee_attendance_adjustments.id as id', 'employee_attendance_adjustments.date as date', 'employee_attendance_adjustments.riderlied_reason as riderlied_reason', 'employee_attendance_adjustments.rejected_reason as rejected_reason', 'employee_attendance_adjustments.status as status_id', 'ls.name as status')
            ->where('employee_id', $employee_id)
            ->where('employee_type_id', 2);
        if ($employee_leaves->exists()) {
            $employee_leaves = $employee_leaves->get();
    ...
    public function leave_riderly_v2(Request $request)
    {
        $rules = [
            'from' => ['required'],
            'to' => ['required'],
            ...
            if ($employee->exists()) {
                ...
                if ($request->leave_type == 1) {
                    ...
                    if (!($from_date >= $start_year && $to_date <= $end_year)) {
                        return response()->json(['status' => 1, 'message' => 'Leave Request Can\'t be riderrove']);
                    }
                ...
                if ($diffDays <= 56) {
                    if ($request->leave_type == 1) {
                        if ($employee->leave_count < $diffDays) {
                            return response()->json(['status' => 1, 'message' => 'Exceed Quota: Dear user, Your limit for riderlying leaves is greater than your available Annual Quota.']);
                        } else {
                    ...
                    if ($request->leave_type == 2) {
                        if ($employee->employee_gender_id == 1) {
                            return response()->json(['status' => 1, 'message' => 'Maternity for males : Your gender doesn\'t allow to riderly this leave category.']);
                        }
                    ...
                    if ($request->leave_type == 3) {
                        if ($employee->employee_gender_id == 2) {
                            return response()->json(['status' => 1, 'message' => 'Your gender doesn\'t allow to riderly this leave category.']);
                        }
                        if ($diffDays > $leave_type->count) {
                            return response()->json(['status' => 1, 'message' => 'Exceed Quota: Dear user, Your limit can\'t be exceed from ' . $leave_type->count . ' days']);
                        }
                    ...
                    if ($request->leave_type == 4) {
                        if ($employee->religion_id != 1) {
                            return response()->json(['status' => 1, 'message' => 'Your are not allow to riderly this leave category.']);
                        }
                        if ($diffDays > $leave_type->count) {
                            return response()->json(['status' => 1, 'message' => 'Exceed Quota: Dear user, Your limit can\'t be exceed from ' . $leave_type->count . ' days']);
                        }
        ...
        if ($delivery_notes->exists()) {
            ...
            foreach ($delivery_notes as $delivery_note) {
                ...
                foreach ($delivery_note_shipments as $delivery_note_shipment) {
                    ...
                    $deliveries['reattempt_marked_status'] = ($information['summary']['completed']['shipment_reattempt_marked'] > 0) ? 1 : 0;
                    $deliveries['shipper'] = $shipper_name;
                    $deliveries['refusal_otp'] = (string)$refusal_otp;
                            ...
                            if ($is_updateable == 0) {
                                ...
                                if ($request->has('hub_id')) {
                                    ...
                                    if ($flag) {
                                        ...
                                        ShipmentScanningJourneyController::add($shipment->id, 4, 5, $request->rider_id, null, null, null, 2, $request->latitude, $request->longitude, 'rider');
                                    ...
                                    ShipmentScanningJourneyController::add($shipment->id, 4, 5, $rider_id, null, null ,null,2, $request->latitude, $request->longitude ,'rider');
                                    $consolidation_details = DeliveryController::check_consolidation($shipment->id);
            ...
            if ($settings->setting_value == 1) {
                if ($environment == 'production' || $environment == 'staging') {
                    ...
                    if ($rider) {
                        ...
                        $rider->save();
                        NotificationsController::rider_notification(9, $rider->id, 2, $otp);
    ...
    public function get_shipment_details(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            if ($shipment->exists()) {
                ...
                ShipmentScanningJourneyController::add($shipment->id, 7, 5, $rider_id, null, null, null, 2, $request->latitude, $request->longitude, 'rider');
    ...
    public function return_note_create(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            if ($shipments_count != 0) {
                ...
                return response()->json(['status' => 0, 'create_message' => 'Return note Request has been created successfully & Pending for riderroval']);
            } else {
...
//                dispatch(new ProcessOneLinkExpireDeliveryNote($request->delivery_note_id));
    ...
    public function shipper_registration(Request $request)
    {
        ...
        //return response()->json(['status' => 1, 'message' => 'Error(s) in Input', 'errors' => $request]);
        ...
        if ($validate->fails()) {
            ...
            if(!$dataExist){
                ...
                if($result){
                    return response()->json(['status' => 0, 'create_message' => 'Retail Shipper has been created','abc'=>$city_id]);
                }else {
            ...
            if ($rider->exists()) {
                ...
                if ($rider->status) {
                    ...
                    if($results){
                        ...
                        return response()->json(['status' => 1, 'message' => 'Invalid Pin','pin 1'=>$old_pin,'Pin 2 '=>$rPin,'Results '=>$results]);
    ...
    public function shipper_all(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            $retailShipperInfo = RetailShipperInfo::where('id', $shipper_id);
            if ($retailShipperInfo->exists()) {
                ...
                //trax metarial types
                $packagingMaterialTypes = PackagingMaterialTypes::where('status', 1)->where('category',3)->get();
        ...
        if ($validate->fails()) {
            ...
            $status = 0;
                ...
                $result = TraxRetailShipperFlyerRequest::create([
                    ...
                    'amount' => $amount,
                    'status'=>$status,
                    'trax_centre_name_verification_id'=>$trax_centre_name_verification_id
                ]);
    
                if($result){
                    $request->trax_centre_name_verification_id=$trax_centre_name_verification_id;
                    return response()->json(['status' => 0, 'create_message' => 'Retail Shipper Flyer Request has been created','success'=>$trax_centre_name_verification_id]);
                }else {
    ...
    //
    public function shipper_retail_shipper_name_verifications(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            $shipper_id = $request->shipper_id; 
            $retail_shipper_name_verifications = RetailShipperNameVerification::where('retail_shipper_info_id',$shipper_id)->get();
            if ($retail_shipper_name_verifications) {
                $request_flyer_last_id = TraxRetailShipperFlyerRequest::where('retail_shipper_id', $shipper_id)
                ->orderBy('id', 'desc')
                ->value('trax_centre_id');
                return response()->json(['status' => 0, 'message' => 'Record Found', 'retail_shipper_name_verifications' => $retail_shipper_name_verifications,'last_trax_centre_id'=>$request_flyer_last_id]);
            }else {
                return response()->json(['status' => 1, 'message' => 'Record not found', 'errors' => 'not Exist']);
    ...
    public function shipper_cities(Request $request)
    {
        ...
        if ($cities->exists()) {
            ...
            foreach ($cities as $city) {
                ...
                $detail['id'] = $city->id;
                $detail['name'] = $city->name;
                $detail['business_category_id'] = $city->business_category_id;
...
// Calculate the total sum of the amounts
   ...
   // add validation array for requested parameters
    ...
    public function shippment_book_retail(Request $request){
        ...
        //return response()->json(['status' => 1, 'message' => $user_id]);
        $pickup_address = UserShippingInfo::where('user_id',$shipper_user_id)->where('city_id',$city_id);
        $pickup_address_id = '';
        if($pickup_address->exists()){
            $pickup_address = $pickup_address->first();
        ...
        $discount = 0;// Auth::user()->store->discount;
        $insurance = 0;// Auth::user()->store->insurance;
        $consignee_phone_number_2 = NULL;
        $consignee_email_address = NULL;
        $package_type = FALSE;
        
        $packaging = ($packaging_amount != null) ?  str_replace(',', '', $packaging_amount) : 0;
        $insurance_amount = 0;//($request->insurance_amount != null) ?  str_replace(',', '',$request->insurance_amount) : 0;
        ...
        $same_day_timing_id = NULL;
            ...
            $breadth = null;
            $height = null;
            $rates = RetailRatesCalculationController::rates($shipping_mode_check, $business_category_id, $city_id, $consignee_city_id, $request->trax_box, $discount, $estimated_weight,$insurance_amount,$packaging);


       
        if ($rates['total_charges'] <= 0)
        {
            return response()->json(['status' => 1, 'message' => 'Total charges cannot be less than zero !','val',[$rates,$packaging]]);
        }

       
        if( $rates['charges'] == 0 && $rates['charges_with_discount'] == 0) {
          //  return response()->json(['status' => 1, 'message' => 'Charges should be greater than zero']);
        }
        ...
        $tracking_number = $request->tracking_number;
        if($tracking_number==''){
            $tracking_number = $this->generate_tracking_number($shipment_id, $city_id, $consignee_city_id);
        }
        ...
        if($shipment->charges_mode_id != 2){
            ...
            //RetailUser::where();
            $retail_user_id = 1;
            $category_id = RetailUser::where("id",$retail_user_id)->first()->category;
            $cash_deposit = RetailCashDeposit::whereDate('created_at', $date)->where('category', $category_id)->where('retail_user_id', $retail_user_id)->where('finalize', 0);
            if($cash_deposit->exists()){
                $cash_deposit = $cash_deposit->first();
                $total_shipments = $cash_deposit->total_cn + 1;
                ...
                $cash_deposit = new RetailCashDeposit();
                $cash_deposit->category = $category_id;
        ...
        $retail_reference = new RetailReference();//RetailReference();

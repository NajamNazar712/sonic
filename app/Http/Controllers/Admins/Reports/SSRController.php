...
<<<<<<< Updated upstream
            ...
            $dr_from_id = DB::connection($connection)->table('shipments_journey')->select('id')->where('created_at', '>=', $dr_from);
            if ($dr_from_id->exists()) {
                $dr_from_id = $dr_from_id->first()->id;
                $dr_to_id = DB::connection($connection)->table('shipments_journey')->select(DB::raw('MAX(id) as id'))->where('created_at', '>=', $dr_from)->where('created_at', '<=', $dr_to);
                if ($dr_to_id->exists()) {
                    $dr_to_id = $dr_to_id->first()->id;
                    $sales->where('dr.id', '>=', $dr_from_id)
                        ->where('dr.id', '<=', $dr_to_id);
                }
            }

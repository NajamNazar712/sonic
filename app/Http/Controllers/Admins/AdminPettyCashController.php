...
use App\Http\Models\Admin\AdvancePettyCashStatement;
...
use App\Http\Models\CostCentre;
...
class AdminPettyCashController extends Controller
{
    ...
    public function make_petty_cash_statement_index()
    {
        ...
        $cost_centres = CostCentre::select('id', 'name')->get();
        ...
        return view('admin.petty_cash.make')->with(['heads' => $head, 'sdns'=>$sdns,'zones'=>$zones,'employees'=>$employees,'operation_managers'=>$operation_managers, 'cost_centres' => $cost_centres]);
    }
        ...
        if ($request->has('submit_button')) {
            ...
            if ($request->input('submit_button') == 'create') {
                ...
                foreach ($selected_ids as $selected_id) {
                    ...
                    $petty_detail->cost_centre_id = $request->cost_centre[$selected_id];
                    $petty_detail->save();
                ...
                foreach ($selected_ids as $selected_id) {
                    ...
                    $petty_detail_draft->cost_centre_id = $request->cost_centre[$selected_id];
...
//                        $file->move(public_path('uploads/petty_cash'), $filename);
    ...
    public function edit_petty_cash_statement_index(Request $request, $id)
    {
        ...
        return view('admin.petty_cash.edit')->with(['heads' => $head, 'petty_statement' => $petty,'zones'=>$zones,'sdns'=>$sdns,'employees'=>$employees,'operation_managers'=>$operation_managers,'hub_array'=>$hub_array, 'cost_centres'=> $cost_centres]);
    }
...
//                    $dropdown .= '<button type="button" class="dropdown-item reference_document" ><div class="row no-gutters align-items-center"><div class="col-2"><i class="ft-edit"></i></div><div class="col-9 offset-1">Reference Document</div></button>';
            ...
            ->addColumn('cost_centre', function ($petty_details) {
                ...
                foreach ($cost_centres as $centre) {
                    if ($centre->id == $petty_details->cost_centre_id) {
                        $selected = 'selected';
                    } else {
                        $selected = '';
                    }
                    $drops .= '<option value="' . $centre->id . '" ' . $selected . '>' . $centre->name . '</option>';
                }
                $select = '<select class="form-control form-control-sm select2 cost_centre_select" disabled  name="cost_centre[' . $petty_details->statement_detail_id . ']" data-rule-required="true" data-msg-required="Cost Centre is required">' . $drops . '</select>';
                return $select;
    ...
    public function reference_document($reference_document)
    {
        ...
        if ($file) {
            ...
            $url = Storage::disk('s3')->url('sonic-archive/petty_cash_statement_details/' . $reference_document);
        }
...
//                if((session('role_id') == 1) || ($petty->status == 0 && (session('role_id') == 9) || session('role_id') == 10) || ($petty->status == 1 && (session('role_id') == 3) || session('role_id') == 8 || session('role_id') == 20) || ($petty->status == 2 && (session('role_id') == 2 || session('role_id') == 7 || session('role_id') == 14))){
    ...
    public function approved_petty_cash_statements_list(Request $request)
    {
        ...
        $petty = PettyCashStatement::leftjoin('cities as h', 'h.id', '=', 'petty_cash_statements.hub_id')
            ->leftjoin('cities as o', 'o.id', '=', 'petty_cash_statements.origin_hub_id')
            ->leftjoin('zones as z', 'z.id', '=', 'o.zone_id')
            ->leftjoin('cities as d', 'd.id', '=', 'petty_cash_statements.destination_hub_id')
            ->join('admins as cb', 'cb.id', '=', 'petty_cash_statements.created_by')
            ->leftjoin('admins as sab', 'sab.id', '=', 'petty_cash_statements.station_approved_by')
            ->leftjoin('admins as oab', 'oab.id', '=', 'petty_cash_statements.operation_approved_by')
            ->leftjoin('admins as fab', 'fab.id', '=', 'petty_cash_statements.finance_approved_by')
            ->leftjoin('shipments', 'shipments.id', '=', 'petty_cash_statements.shipment_id')
            ->select('petty_cash_statements.id as statement_id', 'petty_cash_statements.id as statement_link', 'h.name as hub_name', 'petty_cash_statements.reference_no','z.name as origin_zone','o.name as origin_hub_name','d.name as destination_hub_name', 'petty_cash_statements.from', 'petty_cash_statements.to', 'cb.name as created_by', 'petty_cash_statements.created_at as created', 'sab.name as station_approved_by', 'petty_cash_statements.station_approved_at', 'oab.name as operation_approved_by', 'petty_cash_statements.operation_approved_at', 'fab.name as finance_approved_by', 'petty_cash_statements.finance_approved_at', 'petty_cash_statements.status', 'petty_cash_statements.total_amount','petty_cash_statements.date')
    ...
    public function approved_petty_cash_statements_view(Request $request,$id)
    {
        ...
        $cost_centres = CostCentre::select('id', 'name')->get();
        return view('admin.petty_cash.view')->with(['heads' => $head, 'cities' => $cities, 'petty_statement' => $petty, 'cost_centres'=> $cost_centres]);
    }
    ...
    public function approved_petty_cash_statements_view_list(Request $request,$id)
    {
        $petty_details = PettyCashStatementDetail::leftjoin('cities as h', 'h.id', '=', 'petty_cash_statement_details.hub_id')
            ...
            'petty_cash_statement_details.edit_at as edit_at', 'c.id as city_id', 'a.id as employee_id', 'a.name as employee_name', 'a.designation as employee_designation', 'cost_centres.name as cost_centre', 'petty_cash_statement_details.cost_centre_id')
        ...
        return Datatables::of($petty_details)   
            ...
            ->addColumn('action', function ($petty) {
                ...
                if ((session('role_id') == 1) || in_array(840, session('permissions'))) {
                    $dropdown .= '<button data-account_title_id="'.$petty->account_title_id.'" data-account_remarks="'.$petty->remarks.'" data-account_reference_no="'.$petty->reference_no.'" data-account_employee_trax_id="'.$petty->employee_trax_id.'" data-account_city_id="'.$petty->city_id.'" data-account_expense_details="'.$petty->expense_details.'" data-account_head_id="'.$petty->account_head_id.'" data-id="'.$petty->statement_detail_id.'" data-cost_centre_id="'.$petty->cost_centre_id.'" data-target="#edit_petty_cash_fields" data-toggle="modal" type="button" class="dropdown-item edit_fields" ><div class="row no-gutters align-items-center"><div class="col-2"><i class="ft-edit"></i></div>Edit</button>';
                }
    ...
    public function draft_edit_petty_cash_statement_index(Request $request, $id)
    {
        ...
        if ($draft) {
            ...
            return view('admin.petty_cash.draft.edit')->with(['heads' => $head, 'titles' => $titles_array, 'petty_statement_draft' => $draft,'zones'=>$zones,'employees'=>$employees,'sdns'=>$sdns,'hub_array'=>$hub_array,'city_array'=>$city_array,'operation_managers'=>$operation_managers,'dncc_array'=>$dncc_array, 'cost_centres' => $cost_centres]);
        } else {
    ...
    public function draft_edit_petty_cash_statement_list(Request $request, $id)
    {
        ...
        return Datatables::of($petty_details)
            ...
            ->addColumn('cost_centre', function ($petty_details) {
                ...
                $select = '<select class="form-control form-control-sm select2 cost_centre_select" name="cost_centre[' . $petty_details->petty_cash_statement_draft_id . ']" data-rule-required="true" data-msg-required="Cost Centre is required">' . $drops . '</select>';
                return $select;
            ...
            if ($petty_cash_draft) {
                if ($request->input('submit_button') == 'draft') {
                    foreach ($selected_ids as $selected_id) {
                        ...
                        $petty_cash_draft_detail->cost_centre_id = $request->cost_centre[$selected_id];
    ...
    public function edit_petty_cash(Request $request)
    {
        $validator = Validator::make($request->all(),
            [
                'centre_id' => 'required|int|max:255',
            ...
            [
                'centre_id.required' =>'The Cost Centre field is required',
        ...
        $cost_centre_id = $request->centre_id;
        ...
        if ($existing_petty_detail->exists()) {
            ...
            $update_petty_cash_detail = PettyCashStatementDetail::where('id', $petty_cash_detail_id)
                ->update(['cost_centre_id' => $cost_centre_id,'account_head_id' => $head_id, 'account_title_id' => $title_id, 'city_id' => $city_id, 'expense_details' => $expense_details, 'employee_id' => $employee_id->id, 'reference_no' => $reference_no, 'remarks' => $remarks, 'edit_by'=>Auth::id(), 'edit_at'=>Carbon::now()]);
    ...
    public function edit_petty_cash_amount(Request $request)
    {
        ...
        if ($existing_petty_detail->exists()) {
            ...
            $sdn_modified_adjusted_amount = ($sdn_amount->adjustment_amount - $existing_petty_cash->total_amount ) + $sdn_link->total_amount;
    ...
    {  
        if ($request->has('submit_button')) {
            ...
            if ($request->input('submit_button') == 'create') {
                ...
                $from = $request->select_statement_date_formatted;
                $to = Carbon::parse($from)->endOfDay()->addDays(30)->toDateTimeString();

                $petty_cash = new AdvancePettyCashStatement();
                $petty_cash->zone_id = $request->select_statement_zone;
                $petty_cash->hub_id = $request->select_statement_hub;
                $petty_cash->reference_no = $request->reference_no;
                $petty_cash->from = $from;
                $petty_cash->to = $to;
                $petty_cash->sdn_id = $request->select_statement_sdn;
                $petty_cash->origin_hub_id = Auth::user()->default_hub_id ?? 0;
                $petty_cash->destination_hub_id = Admin::find($request->select_statement_station_manager)->default_hub_id ?? 0;
    ...
    public function advance_petty_cash_statements_index()
    {
        //        ActivityTrailController::createActivityTrailLog(Auth::id(),39);
        if (session('role_id') == 1) {
            $hubs = City::where('hub', 1)->where('status',1)->get();
    ...
    public function advance_petty_cash_statements_list(Request $request)
    {
        ...
        if (session('role_id') != 1) {
            $petty = $petty->where(function ($query) {
                $query->whereIn('advance_petty_cash_statements.origin_hub_id', session('hubs'))
                    ->orWhereIn('advance_petty_cash_statements.destination_hub_id', session('hubs'))
        ...
        $petty = Datatables::of($petty)
            ...
            ->addColumn('sdn_update_logs', function ($petty) {
                $log = AdvacncePettyCashSdnLog::where('petty_cash_statement_id',$petty->statement_id);
                if($log->exists())
                {
                    return '<button class="btn btn-sm btn-outline-info align-middle sdn_logs font-medium-1"><i class="ft-align-justify align-middle"></i></button>';
                }
    ...
    public function advance_add_petty_cash_statement_make_detail(Request $request,$id)
    {
        ...
        return view('admin.petty_cash.advance.make_detail')->with(['heads' => $head, 'petty_statement' => $petty,'zones'=>$zones,'sdns'=>$sdns,'employees'=>$employees,'operation_managers'=>$operation_managers,'hubs'=> $hubs,'hub_array' => $hub_array, 'cost_centres' => $cost_centres]);
    ...
    public function advance_edit_petty_cash_statement_make_detail(Request $request,$id)
    {
        $petty = AdvancePettyCashStatement::find($id);
        if(!in_array($petty->status_id ,[0,1,2,7,6]))
        {
        ...
        return view('admin.petty_cash.advance.edit_make_detail')->with(['heads' => $head, 'petty_statement' => $petty,'zones'=>$zones,'sdns'=>$sdns,'employees'=>$employees,'operation_managers'=>$operation_managers,'hub_array'=>$hub_array, 'cost_centres' => $cost_centres]);
    }
...
//                'z.name as zone',
                ...
                'advance_petty_cash_statement_details.dncc_id','advance_petty_cash_statement_details.delivered_shipments','advance_petty_cash_statement_details.cost_centre_id')
            ->where('advance_petty_cash_statement_details.petty_cash_id', $id);

//            dd($petty_details->get());
        return Datatables::of($petty_details)
            ->setRowAttr([
                'status' => function ($petty_details) {
                    return $petty_details->status;
...
//                    $statement->save();
    ...
    public function advance_statement_print(Request $request)
    {
        $statement_id = $request->id;
        $generator = new \Picqer\Barcode\BarcodeGeneratorPNG();
        ...
        $html = '
                ...
                <html lang="en">
                  ...
                  <body>
                    <div>
      ';
        $petty_cash_statement = AdvancePettyCashStatement::where('id', $statement_id);
        if ($petty_cash_statement->exists()) {
            $total_statements = 0;
            $statement_details = AdvancePettyCashStatementDetail::where('petty_cash_id', $statement_id)->get();
...
//                    dd($petty_cash_statement);
    ...
    public function advance_sdn_log(Request $request)
    {
        $logs = AdvacncePettyCashSdnLog::where('petty_cash_statement_id',$request->statement_id);
        if($logs->exists())
        {
            $logs = $logs->get();
            $data = array();

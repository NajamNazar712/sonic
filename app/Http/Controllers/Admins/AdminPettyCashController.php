...
class AdminPettyCashController extends Controller
{
    ...
    public function make_petty_cash_statement_index()
    {
        ...
        if (session('role_id') == 1) {
            $hub_cities = City::where('hub', 1)->select('id', 'name')->get();
        } else {
            $hub_cities = City::where('hub', 1)->whereIn('id', session('hubs'))->select('id', 'name')->get();
        }
...
//                    $petty_detail->hub_id = ($request->has($hubId) ? $request->hub[$selected_id] : null);
    ...
    public function create_shipment($petty_cash_statement_id)
    {
        ...
        if ($petty_cash_statement_id) {
            ...
            $shipment = $this->book($user_id, 1, $pickup_address_id, 1, $consignee_city_id,$consignee_name, 'Trax Office '.$consignee_city_name, '0213-8772222', NULL, $user->email, NULL, 0, Carbon::now(), $special_instructions, 1, 1, NULL, 0, 1, 1, 1);

...
//                        DB::raw('(select id from shipments_journey where shipments_journey.shipment_id = shipments.id and shipments_journey.shipper_status_id = 13)'));
        ...
        $datatable = Datatables::of($shipments)
            ...
            ->addColumn('confirmation_on',function ($result){
                $status_date = Carbon::parse($result->last_status_date);
                $tat_date = Carbon::parse($result->last_status_date)->addDays($result->tat_value);
                $tat_date->diffInDays($status_date)

                return $status_date;
            })
            ->addColumn('confirmation_req',function ($result){
                return $result->confirmation_on;
            })
            ...
            ->addColumn("action", function ($result) {
                ...
                if($result->assigned_agent_id == Auth::id() || $result->tat < 1 || (in_array(490, session('permissions')))) {
                ...
                foreach ($shipment_ids as $shipment_id) {
                    ...
                    if(!ReturnNoteShipment::join('return_notes', 'return_notes.id', '=', 'return_note_shipments.return_note_id')->where('return_note_shipments.return_note_id','>', $request->return_note_id)->where('shipment_id', $shipment_id)->exists()) {
                        ...
                        if ($parcel->booking_type_id == 2) {
                            ShipmentsJourneyController::add($shipment_id, 31, 31, NULL, ($request->has($shipment_remark) ? $request->remarks[$shipment_id] : null), NULL, Auth::id(), $request->return_note_id, NULL, 1, ($request->has($received_or_refused_by) ? $request->received_or_refused_by[$shipment_id] : null));

...
class ReturnController extends Controller
{
    ...
    private function shipments(){
       ...
       ->leftJoin('rider_deliveries', function ($join) {
        ...
        'shipments_journey.created_at as status_date','shipments_journey.created_at as last_status_date','sj.created_at as arrival', 'shipments.booking_type_id',
         ...
         'rvsaa.unresponsive_count as rvsaa_unresponsive_count','rvsaa.unresponsive_attempt_time as unresponsive_attempt_time','rach.created_at as call_time', 'rvsaa.rv_state_id as rv_state_id', 'rvsaad.agent_id as last_agent_name', 'delivery_notes.pending_status as delivery_note_pending_status')
    ...
    public function return_view()
    {
        ...
        $oldest_shipments = $total_of_shipments - count(RvShipmentAssignAgent::leftJoin('shipments','rv_shipment_assign_agents.shipment_id','shipments.id')->leftJoin('shipments_journey', function ($join) {
            $join->on('shipments_journey.shipment_id', '=', 'shipments.id')
            ->where('shipments_journey.id','=',
            DB::raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = shipments.id)'));
        })->where('shipments_journey.shipper_status_id', 12)->whereDate('shipments_journey.created_at', '<', Carbon::today())->whereNotNull('rv_shipment_assign_agents.shipment_id')->get());
        //Average Aging

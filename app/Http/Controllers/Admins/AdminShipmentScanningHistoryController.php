...
//                }
            ...
            if($flag == true){
                if(count($scanning_histories) > 0){
                    foreach($scanning_histories as $index => $scanning_history){
                        ...
                        $details[$index]['area_log'] = in_array($scanning_history->screen_location_id, [1, 2, 20, 21, 4, 3, 10, 7, 31 , 26, 27, 30, 8]) ? ShipmentScanningJourneyAreaLog::where('shipment_scanning_journey_id', $scanning_history->id)->first() : [];
    ...
    public function details_new(Request $request){
        ...
        return view('admin.scanning_history.index_new', compact('data'));
    }
}

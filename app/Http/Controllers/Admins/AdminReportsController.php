...
class AdminReportsController extends Controller
{
    ...
    public function revenue_list(Request $request)
    {
        ...
        $sales = DB::connection($connection)->table('shipments')
            ...
            ->leftJoin(DB::raw("(SELECT shipment_id, SUM(amount) as d_collection_amount, SUM(gst) as d_gst, SUM(charges) as d_total_charges, SUM(payable) as d_net_payable, SUM(sms_charges) as dps_sms_charges FROM done_payment_shipments WHERE type != 2 GROUP BY shipment_id) as dps"), function($join) { $join->on('dps.shipment_id', '=', 'shipments.id'); })
            ...
            ->leftJoin(DB::raw("(SELECT shipment_id, SUM(gst) as is_gst, SUM(sms_charges) as is_sms_charges FROM invoice_shipments WHERE type != 2 GROUP BY shipment_id) as invoice_summary"), function($join) { $join->on('invoice_summary.shipment_id', '=', 'shipments.id'); })

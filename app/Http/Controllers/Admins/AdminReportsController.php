...
//        }
    ...
    public static function revenue_excel_download(){
        ...
        $from = Carbon::today()->subMonth(2)->firstOfMonth()->startOfDay()->toDateTimeString();
        $to = Carbon::today()->subMonth(2)->endOfMonth()->subDays(15)->toDateTimeString();
        $sales = DB::connection('reports')->table('shipments')->join('users as u','u.id','=','shipments.user_id')
            ->join('shipment_status as ss','ss.id','=','shipments.shipper_status_id')
            ->join('booking_types as bt','bt.id','=','shipments.booking_type_id')
            ->join('user_shipping_infos AS usi', 'shipments.pickup_address_id', '=', 'usi.id')
        ...
        $filename = 'sonic_monthly_shipper_revenue_report_june.xlsx';

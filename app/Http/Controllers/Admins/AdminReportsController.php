...
//    }
    ...
    public function last_mile_app_list(Request $request){
        ...
        $deliveries = DB::connection('reports')->table('delivery_notes')
            ...
            ->select('delivery_notes.id as delivery_note_id', 'delivery_notes.created_at as created_at', 'r.name as rider', 'delivery_notes.shipments_count as total_shipments', 'c.name as city', DB::raw('(SELECT COUNT(distinct shipment_id) as id FROM `rider_deliveries` AS `rd` where `rd`.`delivery_note_id` = `delivery_notes`.`id`) AS `shipments_rider_updated`') , DB::raw('(SELECT COUNT(distinct shipment_id) as id FROM `shipments_journey` AS `sj` where `sj`.`reference_1_id` = `delivery_notes`.`id` AND `sj`.`shipper_status_id` != 5 AND `sj`.`rider_id` is null AND `sj`.`verification` = 1) AS `shipments_dbf_updated`'));

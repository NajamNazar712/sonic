    ...
    public function revenue_list(Request $request)
    {
        ...
        $sales = DB::connection($connection)->table('shipments')
            ...
            ->leftJoin(DB::raw("
                    ...
                    ) as pps"), function($join) {$join->on('pps.shipment_id', '=', 'shipments.id');
            })
            ->leftJoin(DB::raw("
                    (SELECT shipment_id, 
                     ...
                     WHERE type != 2
                     GROUP BY shipment_id
                    ) as dps"), function($join) {$join->on('dps.shipment_id', '=', 'shipments.id');
            })
            ->leftJoin(DB::raw("
                    (SELECT shipment_id, 
                            SUM(gst) as pis_gst,
                     ...
                     WHERE type != 2
                     GROUP BY shipment_id
                    ) as pis"), function($join) {$join->on('pis.shipment_id', '=', 'shipments.id');
            })
            ->leftJoin(DB::raw("
                (SELECT shipment_id, 
                        SUM(gst) as is_gst,

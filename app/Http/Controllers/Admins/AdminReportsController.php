...
class AdminReportsController extends Controller
{
    ...
    public function qsr_list(Request $request){
        ...
        if ($type && $type == 2) {
            $shipments = $shipments->whereNotIn('shipments.shipper_status_id',[1,14,16,17,25,31,36,38,39,40,41,43]);
        }
        else {
            $shipments = $shipments->whereNotIn('shipments.shipper_status_id',[1,14,16,17,25,31,36,38,39,40,41,43,47,51]);
        }
    ...
    static public function daily_pickup_sales_report_create($search_city = NULL, $date,$sales_person = NULL,$sales_tagging = FALSE){
        ...
        if($sales_tagging == TRUE) {
            ...
            if (session('role_id') == 1) {
                if ($search_city != null) {
                    ...
                    $city = DB::connection('mysql')->table('cities')->where('id', $search_city)->select('id', 'name')->first();
                    $search_city_hub = $city->id;
                    $hubs[] = $city;
                } else {
                    $search_city_hub = '';
                    $hubs = DB::connection('mysql')->table('cities')->where('pickup', 1)->select('id', 'name')->get();
                }
            } else {
                if ($search_city != null) {
                    ...
                    $search_city_hub = '';
                    $hubs = DB::connection('mysql')->table('cities')->whereIn('hub_id', session('hubs'))->where('pickup', 1)->select('id', 'name')->get();
                }
            }
        }else{
            $search_city_hub = '';
        ...
        foreach ($hubs as $hub) {
            if($sales_tagging == TRUE){
                ...
                if (session('department_id') != 7){
                    $booked = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    $received = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    $cod_collection = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    $actual_weight = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    $chargeable_weight = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    $revenue_wo_gst = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function($sub_query) use ($hub) {
                                $sub_query->from('cities')
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->sum(DB::connection('mysql')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
                    ...
                    if(session('role_id') != 4){
                        ...
                        $revenue_wo_gst = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                            ...
                            $query->from('shipments_journey')
                                ...
                                ->whereBetween('created_at', [$date_from, $date_to])
                                ->where('shipper_status_id', 2);
                        })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->sum(DB::connection('mysql')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
                    }else{
                        if($sales_person != null){
                            $tagged_shippers = DB::connection('mysql')->table('sale_person_tags')->where('admin_id', $sales_person)->select('user_id')->pluck('user_id')->toArray();

                            $assigned_admins = DB::connection('mysql')->table('multiple_sale_leads')->leftjoin('multiple_sale_taggings as mst', 'mst.lead_id', '=', 'multiple_sale_leads.id')
                            ...
                            $revenue_wo_gst = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($hub) {
                                ...
                                $query->from('shipments_journey')
                                    ...
                                    ->whereBetween('created_at', [$date_from, $date_to])
                                    ->where('shipper_status_id', 2);
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('user_id', $tagged_shippers)->sum(DB::connection('mysql')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
...
//        $details_shipper['header'] = ['S. No.','DSR '.$only_date, 'No. of Parcels Booked','No of Parcels Received','Revenue without GST','Collection Amount','Actual Weight','Chargeable Weight','Avg/Parcel Revenue','Avg. Amount Collection','% Rev. on Amount Collection'];
        ...
        if($sales_tagging == TRUE){
            if($search_city != null){
                $pickup_request_shippers = DB::connection('mysql')->table('shipments')->whereExists(function($query) use ($search_city_hub) {
                    $query->from('user_shipping_infos')
                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                ...
                if($pickup_request_shippers->exists()){
                    ...
                    if(session('department_id') != 7){
                        $shippers[$search_city_hub] = DB::connection('mysql')->table('users')->select('id','name')->whereIn('id',$pickup_request_shippers_ids)->whereIn('status',[3, 4])->get();
                    }else{
                        if(session('role_id') != 4){
                            $shippers[$search_city_hub] = DB::connection('mysql')->table('users')->select('id','name')->whereIn('id',$pickup_request_shippers_ids)->whereIn('status',[3, 4])->whereIn('id', session('tagged_shippers'))->get();
                        }else{
                            $shippers[$search_city_hub] = DB::connection('mysql')->table('users')->select('id','name')->whereIn('id',$pickup_request_shippers_ids)->whereIn('status',[3, 4])->get();
                        }
                    }
                ...
                foreach($hubs as $hub){
                    ...
                    $pickup_request_shippers = DB::connection('mysql')->table('shipments')->whereExists(function ($query) use ($hub) {
                        $query->from('user_shipping_infos')
                            ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                            ->whereExists(function ($sub_query) use ($hub) {
                                $sub_query->from('cities')
                    ...
                    if($pickup_request_shippers->exists()) {
                        ...
                        if (session('department_id') != 7) {
                            ...
                            $shippers[$hub->id] = DB::connection('mysql')->table('users')->select('id', 'name')->whereIn('id', $pickup_request_shippers_ids)->whereIn('status', [3, 4])->get();
//                        }
                        } else {
                            if (session('role_id') != 4) {

                                $shippers[$hub->id] = DB::connection('mysql')->table('users')->select('id', 'name')->whereIn('id', $pickup_request_shippers_ids)->whereIn('status', [3, 4])->whereIn('id', session('tagged_shippers'))->get();
//                            }
                                ...
                                if ($sales_person != null) {
                                    ...
                                    $shippers[$hub->id] = DB::connection('mysql')->table('users')->whereExists(function ($query) use ($sales_person) {
...
//        $shipper_sales_person = '';
        ...
        if(count($shippers) > 0) {
            if($search_city != null){
                $origin_name =  DB::connection('mysql')->table('cities')->where('id', $search_city_hub)->select('name')->first();
            ...
            foreach ($shippers as $origin => $shipper_row) {
                foreach ($shipper_row as $shipper) {
                    ...
                    if($search_city == null){
                        $origin_name =  DB::connection('mysql')->table('cities')->where('id', $origin)->select('name')->first();
                        $origin_name = $origin_name->name;
                    }
                    ...
                    if ($sales_tagging == TRUE) {
                        if ($search_city != null) {
                            $shipper_booked = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                            ...
                            $shipper_received = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                                ->whereExists(function ($query) use ($hubs) {
                                    $query->from('user_shipping_infos')
                                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                        ->whereExists(function ($sub_query) use ($hubs) {
                            ...
                            $shipper_rev_wo_gst = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                                ->whereExists(function ($query) use ($hubs) {
                                    $query->from('user_shipping_infos')
                                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                        ->whereExists(function ($sub_query) use ($hubs) {
                                ...
                                ->whereExists(function ($query) use ($date_from, $date_to) {
                                    $query->from('shipments_journey')
                                        ...
                                        ->whereBetween('created_at', [$date_from, $date_to])
                                        ->where('shipper_status_id', 2);
                                })->where('shipments.packaging_material_request', '=', 0)->sum(DB::connection('mysql')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
                            $shipper_cod = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                                ->whereExists(function ($query) use ($hubs) {
                                    $query->from('user_shipping_infos')
                                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                        ->whereExists(function ($sub_query) use ($hubs) {
                            ...
                            $shipper_actual_weight = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                                ->whereExists(function ($query) use ($hubs) {
                                    $query->from('user_shipping_infos')
                                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                        ->whereExists(function ($sub_query) use ($hubs) {
                            ...
                            $shipper_chargeable_weight = DB::connection('mysql')->table('shipments')->where('user_id', $shipper->id)
                                ->whereExists(function ($query) use ($hubs) {
                                    $query->from('user_shipping_infos')
                                        ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                        ->whereExists(function ($sub_query) use ($hubs) {
...
//                $serial_number_shippers++;
        ...
        $shipping_mode_wise_details = self::daily_pickup_sales_shipping_mode_wise($date_from,$date_to,$search_city, $sales_tagging);
    ...
    public function revenue_list(Request $request){
        ...
        $sales = DB::connection('reports')->table('shipments')->join('users as u','u.id','=','shipments.user_id')
            ...
            ->leftJoin('shipments_journey as dr', function ($join) {
                $join->on('dr.shipment_id', '=', 'shipments.id')
                    ->where('dr.id','=',
                        DB::connection('reports')->raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = shipments.id and shipments_journey.shipper_status_id In(14,20,30,36,37) and verification = 1)'));
            })
                ...
                foreach($shipping_modes as $mode){
                    ...
                    $received = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($city) {
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->where('shipments.shipping_mode_id', $mode->id)->count();
                    ...
                    $revenue_wo_gst = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($city) {
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->where('shipments.shipping_mode_id', $mode->id)->sum(DB::connection('reports')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
                    ...
                    $cod_collection = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($city) {
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->where('shipments.shipping_mode_id', $mode->id)->sum('amount');
                    ...
                    $actual_weight = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($city) {
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->where('shipments.shipping_mode_id', $mode->id)->sum('actual_weight');
                    ...
                    $chargeable_weight = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($city) {
                        ...
                        $query->from('shipments_journey')
                            ...
                            ->whereBetween('created_at', [$date_from, $date_to])
                            ->where('shipper_status_id', 2);
                    })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->where('shipments.shipping_mode_id', $mode->id)->sum('chargeable_weight');
                ...
                if((session('department_id') == 7) && (session('role_id') != 4)){
                    ...
                    foreach($shipping_modes as $mode){
                        ...
                        $booked= DB::connection('reports')->table('shipments')
                            ...
                            ->where('shipments.shipping_mode_id', $mode->id)->whereIn('shipments.user_id', session('tagged_shippers'))->count();
                        ...
                        $received = DB::connection('reports')->table('shipments')
                            ->whereExists(function ($query) use ($date_from, $date_to) {
                                $query->from('shipments_journey')
                                    ...
                                    ->whereBetween('created_at', [$date_from, $date_to])
                                    ->where('shipper_status_id', 2);
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->where('shipments.shipping_mode_id', $mode->id)->count();

                        $revenue_wo_gst = DB::connection('reports')->table('shipments')->whereExists(function ($query) use ($date_from, $date_to) {
                            $query->from('shipments_journey')
                                ->where('shipments.id', DB::raw('`shipments_journey`.`shipment_id`'))
                                ->whereBetween('created_at', [$date_from, $date_to])
                                ->where('shipper_status_id', 2);
                        })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->where('shipments.shipping_mode_id', $mode->id)->sum(DB::connection('reports')->raw('IFNULL(weight_charges,0) + IFNULL(cash_handling_charges,0) + IFNULL(insurance_charges,0) + IFNULL(return_charges,0) + IFNULL(fuel_surcharge,0) + IFNULL(replacement_charges,0) + IFNULL(try_and_buy_charges,0)'));
                        ...
                        $cod_collection = DB::connection('reports')->table('shipments')
                            ->whereExists(function ($query) use ($date_from, $date_to) {
                                $query->from('shipments_journey')
                                    ...
                                    ->whereBetween('created_at', [$date_from, $date_to])
                                    ->where('shipper_status_id', 2);
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->where('shipments.shipping_mode_id', $mode->id)->sum('amount');
                        ...
                        $actual_weight = DB::connection('reports')->table('shipments')
                            ->whereExists(function ($query) use ($date_from, $date_to) {
                                $query->from('shipments_journey')
                                    ...
                                    ->whereBetween('created_at', [$date_from, $date_to])
                                    ->where('shipper_status_id', 2);
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->where('shipments.shipping_mode_id', $mode->id)->sum('actual_weight');

                        $chargeable_weight = DB::connection('reports')->table('shipments')->whereExists(function ($query) use ($date_from, $date_to) {
                            $query->from('shipments_journey')
                                ->where('shipments.id', DB::raw('`shipments_journey`.`shipment_id`'))
                                ->whereBetween('created_at', [$date_from, $date_to])
                                ->where('shipper_status_id', 2);
                        })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->where('shipments.shipping_mode_id', $mode->id)->sum('chargeable_weight');
                    ...
                    foreach($shipping_modes as $mode){
                        ...
                        $booked= DB::connection('reports')->table('shipments')
                            ->whereBetween('shipments.created_at',[$date_from,$date_to])
                            ->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id','!=',1690)
                            ->where('shipments.shipping_mode_id', $mode->id)->count();

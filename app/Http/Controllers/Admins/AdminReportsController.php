...
//        }
    ...
    public function get_city_shippers(Request $request){
        $city_id=$request->get('city_id');
        $shippers=User::select('id','name')->where('status',3)->where('city_id',$city_id)->get();
        return response()->json(['status'=>0,'shippers'=>$shippers]);
    }
    ...
    public function created_shipment_list(Request $request){
        ...
        if($shipper_id)
        {
            $shipment_picked->where('s.user_id',$shipper_id);
        }
    ...
    public function pickup_arival_list(Request $request){
        ...
        $pickup_arival=DB::connection('reports')->
        ...
        $datatables = Datatables::of($pickup_arival)
            ...
            ->addColumn('shipment_balance_btn', function ($pickup_arival) {
                if ($pickup_arival->shipment_arrived > 0 ||  $pickup_arival->rider_picked >0 ) {
                         $balance=abs(($pickup_arival->shipment_arrived-$pickup_arival->rider_picked));
    ...
    public function get_shipper_addresses(Request $request){
        ...
        ->select('usi.pickup_address',DB::raw('COUNT(shipments.id) as booked_shipments'))->where('shipments.user_id', $shipper_id)
        ->whereRaw("DATE(shipments.created_at) BETWEEN '$from_date' AND '$to_date'")
        ->groupBy('shipments.pickup_address_id')->get();

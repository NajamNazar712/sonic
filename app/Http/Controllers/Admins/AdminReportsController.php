...
class AdminReportsController extends Controller
{
    ...
    public function qsr_list(Request $request){
        ...
        $shipments = DB::connection('reports')->table('shipments')->join('users as u', 'shipments.user_id', '=', 'u.id')
            ...
            ->leftjoin('shipment_status_reason as ssr','ssr.id','=','journey.status_reason_id')//,'ssr.name as reason'
            ->leftjoin('shipment_items as si', function ($join) {
                $join->on('si.shipment_id', '=', 'shipments.id')
                    ->where('si.type','=',0);
            })
            ->leftjoin('products as p','p.id','=','si.product_type_id')//'ssr.name as reason',
            ->select(['z.name  as zone','p.product_name as product_type','si.description as description','ssr.name as reason','shipments.id as shId','shipments.tracking_number','shipments.tracking_number as tracking_number_link','u.name as shipper','ss.name as history_status','bt.booking_type as service_type','sj.created_at as arrival','oc.name as origin','dc.name as destination','h.name as hub','shipments.amount','journey.created_at as last_status_date','shipments.consignee_name as name', 'shipments.booking_type_id', 'shipments.created_at', 'usi.poc','u.id as account_no','sm.mode as shipping_mode', 'shipments.order_id as order_id']);
        ...
        foreach ($hubs as $hub) {
            if($sales_tagging == TRUE){
                ...
                if (session('department_id') != 7){
                    ...
                    })->whereBetween('shipments.created_at',[$date_from,$date_to])->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->select('shipments.id')->get()->count();
                    ...
                    if(session('role_id') != 4){
                        ...
                        })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereBetween('created_at',[$date_from,$date_to])->whereIn('shipments.user_id', session('tagged_shippers'))->select('shipments.id')->get()->count();
                        $received = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($hub) {
                            $query->from('user_shipping_infos')
                                ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                ->whereExists(function($sub_query) use ($hub) {
                            ...
                            $query->from('shipments_journey')
                                ...
                                ->whereBetween('created_at', [$date_from, $date_to])
                                ->where('shipper_status_id', 2);
                        })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('shipments.user_id', session('tagged_shippers'))->select('shipments.id')->get()->count();
                        $cod_collection = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($hub) {
                            $query->from('user_shipping_infos')
                                ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                ->whereExists(function($sub_query) use ($hub) {
                        ...
                        if($sales_person != null){
                            ...
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereBetween('created_at',[$date_from,$date_to])->whereIn('user_id', $tagged_shippers)->select('shipments.id')->get()->count();
                            ...
                            $received = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($hub) {
                                ...
                                $query->from('shipments_journey')
                                    ...
                                    ->whereBetween('created_at', [$date_from, $date_to])
                                    ->where('shipper_status_id', 2);
                            })->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id', '!=', 1690)->whereIn('user_id', $tagged_shippers)->select('shipments.id')->get()->count();
                            $cod_collection = DB::connection('reports')->table('shipments')->whereExists(function($query) use ($hub) {
                                $query->from('user_shipping_infos')
                                    ->where('shipments.pickup_address_id', '=', DB::raw('`user_shipping_infos`.`id`'))
                                    ->whereExists(function($sub_query) use ($hub) {
...
//        }
    ...
    public function last_mile_status_data($from, $to, $destination, $hub, $zone, $rider){
        ...
        foreach ($time_slots as $id => $slot){
            ...
            $time_array = array();
            $total_shipment_deliverd_bolt = DeliveryNoteShipment::where([ 'update_type' => 1])->where('status',6)->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)->whereTime('created_at', '>=', $start_time)->whereTime('created_at', '<=', $end_time)->count();
            $total_shipment_undeliverd_bolt = DeliveryNoteShipment::where([ 'update_type' => 1])->where('status','!=',6)->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)->whereTime('created_at', '>=', $start_time)->whereTime('created_at', '<=', $end_time)->count();
            $total_shipment_deliverd_sonic = DeliveryNoteShipment::where([ 'update_type' => 0])->where('status',6)->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)->whereTime('created_at', '>=', $start_time)->whereTime('created_at', '<=', $end_time)->count();
            $total_shipment_undeliverd_sonic = DeliveryNoteShipment::where([ 'update_type' => 0])->where('status','!=',6)->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)->whereTime('created_at', '>=', $start_time)->whereTime('created_at', '<=', $end_time)->count();
            ...
            if($total_status_updated_count > 0 && $out_for_delivery_percentage > 0){
            ...
            $time_array['total_shipment_deliverd_bolt'] =$total_shipment_deliverd_bolt;
            $time_array['total_shipment_undeliverd_bolt'] =$total_shipment_undeliverd_bolt;
            $time_array['total_shipment_deliverd_sonic'] =$total_shipment_deliverd_sonic;
            $time_array['total_shipment_undeliverd_sonic'] =$total_shipment_undeliverd_sonic;
...
//                $sum_sonic_status_percentage = $sum_sonic_status_percentage + $sonic_status_percentage;
            $sum_total_shipment_deliverd_bolt = $sum_total_shipment_deliverd_bolt + $total_shipment_deliverd_bolt;
            $sum_total_shipment_undeliverd_bolt = $sum_total_shipment_undeliverd_bolt + $total_shipment_undeliverd_bolt;
            $sum_total_shipment_deliverd_sonic = $sum_total_shipment_deliverd_sonic + $total_shipment_deliverd_sonic;
            $sum_total_shipment_undeliverd_sonic = $sum_total_shipment_undeliverd_sonic + $total_shipment_undeliverd_sonic;
        ...
        if($sales_tagging == true){
            ...
            if($hub != null){
                ...
                foreach($shipping_modes as $mode){
                    ...
                    })->whereBetween('shipments.created_at',[$date_from,$date_to])->where('shipments.packaging_material_request', '=', 0)->where('shipments.user_id','!=',1069)->where('shipping_mode_id', $mode->id)->select('shipments.id')->get()->count();

...
//            $not_delivered_statuses = [4,6,7,8,9,11,12,13,15];
                                ...
                                ->orWhere(function ($sub_query) use ($arrival_cut_off_time, $from) {
                                    $sub_query->where(function ($sub_sub_query) {
                                        $sub_sub_query->where('usi.city_id', '=', DB::connection('reports')->raw('sch.hub_id'))
                                            ...
                                            ->where(DB::raw('DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat + IF ((WEEK(sj.created_at) <> WEEK(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY))) OR (WEEKDAY(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY)) IN (6)), 1 , 0) DAY)'), '<', Carbon::today());
                                    })
                                    ->orWhere(function ($sub_sub_sub_query) {
                                        $sub_sub_sub_query->where('usi.city_id', '!=', DB::connection('reports')->raw('sch.hub_id'))
                                            ...
                                            ->where('CURDATE()', '=', DB::raw('DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat + IF ((WEEK(sj.created_at) <> WEEK(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY))) OR (WEEKDAY(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY)) IN (6)), 1 , 0) DAY)'), '<', Carbon::today());
                                    });
                                });
                            });
                        }
                            ...
                            $rows = $rows->where(function ($query) use ($arrival_cut_off_time, $from) {
                                ...
                                ->orWhere(function ($sub_query) use ($arrival_cut_off_time, $from) {
                                    $sub_query->where(function ($sub_sub_query) {
                                        $sub_sub_query->where('usi.city_id', '=', DB::connection('reports')->raw('sch.hub_id'))
                                            ->whereIn('sj.shipper_status_id', [2,6,7,8,9,11,12,13,15])
                                            ->whereRaw(DB::raw('DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat + IF ((WEEK(sj.created_at) <> WEEK(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY))) OR (WEEKDAY(DATE_ADD(sj.created_at, INTERVAL cities.attempt_tat DAY)) IN (6)), 1 , 0) DAY)'), '>=', Carbon::today());
                                    })
                                    ->orWhere(function ($sub_sub_sub_query) {
                                        $sub_sub_sub_query->where('usi.city_id', '!=', DB::connection('reports')->raw('sch.hub_id'))
                                            ->whereIn('sj.shipper_status_id', [4,6,7,8,9,11,12,13,15])
        ...
        static public function debriefing_hub_wise_report($date){
            $hubs = DB::connection('reports')->table('cities')->where('hub', 1)->pluck('id');
            if($hubs){
                $zone = NULL;
                $mode = NULL;
        ...
        static public function debriefing_zone_wise_report($date){
            ...
            $zone = DB::connection('reports')->table('zones')->where('status', 1)->pluck('id');
            if($zone){
    ...
    //        }
        ...
        public function adjustments_list(Request $request){
            $adjustments = DB::connection('reports')->table('adjustment_logs')
                ...
                ->select('adjustment_logs.id as adjustment_id', 'adjustment_logs.adjustment_amount as adjustment_amount', 'adjustment_logs.remarks as remarks', 's.tracking_number as tracking_number', 'at.name as adjustment_type', 'adjustment_logs.created_at as created_at', 'a.name as created_by', 'u.name as shipper_name', 'dps.done_payment_id as done_payment_id')

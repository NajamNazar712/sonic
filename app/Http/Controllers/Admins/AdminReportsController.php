...
class AdminReportsController extends Controller
{
    ...
    public function qsrold_index(Request $request)
    {
    ...
    public function return_note_list(Request $request)
    {
        ...
        if ($rn_no = $request->get('search_rn_no')) {
            $return_note->where('return_notes.id', '=', $rn_no);
        }
        if ($tracking = $request->get('search_tracking')) {
            $return_note->join('return_note_shipments as rnst', 'rnst.return_note_id', '=', 'return_notes.id')
                ->join('shipments as s', 'rnst.shipment_id', '=', 's.id')
                ->where('s.tracking_number', '=', $tracking);
        }
        if ($rider = $request->get('search_rider')) {
            $return_note->where('riders.id', '=', $rider);
        }
        if ($hub = $request->get('search_hub')) {
            $return_note->where('oc.id', '=', $hub);
        }
        if ($mode = $request->get('search_shipping_mode')) {
            $return_note->where('shipments.booking_type_id', '=', $mode);
        }
        if ($created_by = $request->get('search_created_by')) {
            $return_note->where('cr.id', '=', $created_by);
        }
        if ($submitted_by = $request->get('search_submitted_by')) {
            $return_note->where('up.id', '=', $submitted_by);
        }
        if ($submission_date = $request->get('search_submission')) {
            $return_note->whereDate('return_notes.updated_at', $submission_date);
        }
        if ($request->get('search_date_from') && $request->get('search_date_to')) {
            $from = $request->get('search_date_from');
            $to = $request->get('search_date_to');
            $return_note->whereBetween('return_notes.created_at', [$from, $to]);
        }
        $return = Datatables::of($return_note)
            ...
            })->rawColumns(['return_note_link','delivered_to_shipper_count_link','image','shipments_count_link']);
    ...
    public function pickup_note_list(Request $request)
    {
        ...
        $pickup_note = Datatables::of($pickup_note)
            ...
            })->rawColumns(['pickup_note_no', 'bookings_link']);
    ...
    public function cargo_received_list(Request $request)
    {
        ...
        if ($cargo_no = $request->get('search_cargo_no')) {
            $cargo_received->where('cargo_consignments.id', '=', $cargo_no);
        }
        if ($tracking = $request->get('search_tracking')) {
            $cargo_received->join('cargo_consignment_shipments as ccs', 'ccs.cargo_consignment_id', '=', 'cargo_consignments.id')
                ->join('shipments as s', 'ccs.shipment_id', '=', 's.id')
                ->where('s.tracking_number', '=', $tracking);
        }
        if ($origin = $request->get('search_origin')) {
            $cargo_received->where('oc.id', '=', $origin);
        }
        if ($destination = $request->get('search_destination')) {
            $cargo_received->where('h.id', '=', $destination);
        }
        if ($mode = $request->get('search_shippimg_modes')) {
            $cargo_received->where('sm.id', '=', $mode);
        }
        if ($transit_date = $request->get('search_transit_date')) {
            $cargo_received->whereDate('cargo_consignments.created_at', $transit_date);
        }
        if ($received_date = $request->get('search_received_date')) {
            $cargo_received->whereDate('cargo_consignments.updated_at', $received_date);
        }
        if ($request->get('search_date_from') && $request->get('search_date_to')) {
            $from = $request->get('search_date_from');
            $to = $request->get('search_date_to');
            $cargo_received->whereBetween('cargo_consignments.created_at', [$from, $to]);
        }
        if ($cargo_type = $request->get('cargo_type')) {
            if ($cargo_type != 0) {
                $cargo_received->where('cargo_consignments.type', $cargo_type);
            }
        }
        $cargo = Datatables::of($cargo_received)
            ...
            })->rawColumns(['cargo_id_link','shipments_link']);
    ...
    public function multiple_iban_list(Request $request)
    {
        ...
        $iban_received = DB::connection('reports')->table('user_bank_infos')->join('users as u', 'u.id', '=', 'user_bank_infos.user_id')
            ...
            ->select([
                'user_bank_infos.id as account_id',
                'u.name as shipper',
                'bl.name as bankname',
                'user_bank_infos.bank_branch',
                'user_bank_infos.account_no',
                'user_bank_infos.account_title',
                'oc.name as city',
                'user_bank_infos.iban',
                'user_bank_infos.default_bank as default',
                'user_bank_infos.created_at as bank_added_at'
            ]);
        if ($ibanNo = $request->get('search_iban_no')) {
            $iban_received->where('user_bank_infos.iban', '=', $ibanNo);
        }
        if ($shipper = $request->get('search_shipper')) {
            $iban_received->where('u.id', '=', $shipper);
        }
        if ($request->get('search_date_from') && $request->get('search_date_to')) {
            $from = $request->get('search_date_from');
            $to = $request->get('search_date_to');
            $iban_received->whereBetween('user_bank_infos.created_at', [$from, $to]);
        }
    ...
    public function lead_time_index(Request $request)
    {
        ...
        $shipping_modes = DB::connection('reports')->table('shipping_modes')->get();
        $sub_segments = SubCategorySegment::select('id', 'name')->get();
        return view('admin.reports.lead_time_report')->with(['cities' => $cities, 'statuses' => $statuses, 'hubs' => $hubs, 'shipper' => $shipper, 'shipping_modes' => $shipping_modes, 'sub_segments' => $sub_segments]);
    }
    ...
    public function lead_time_list(Request $request)
    {
        ...
        if ($status = $request->get('search_status')) {
            $shipments->where('ss.id', '=', $status);
        }
        if ($mode = $request->get('search_shipping_mode')) {
            $shipments->where('shipments.shipping_mode_id', '=', $mode);
        }
        if ($sub_segment = $request->get('sub_segment')) {
    ...
    public function outstanding_shipments_list(Request $request)
    {
        ...
        if ($recovery_status = $request->get('search_recovery_status')) {
            if ($recovery_status == 0) {
                $shipments->whereIn('delivery_note_shipments.status', [4, 5, 6, 7, 11]);
            } else if ($recovery_status == 1) {
        ...
        if ($mode = $request->get('search_shipping_mode')) {
            $shipments->where('s.booking_type_id', '=', $mode);
        }
        if ($hub = $request->get('hub')) {
        ...
        $datatables = Datatables::of($shipments)
            ...
            })->rawColumns(['dncc_link','sdn_link','tracking_number_link']);
    ...
    public function completed_delivery_notes_list(Request $request)
    {
        ...
        $deliveries = DB::connection('reports')->table('delivery_notes')->join('cities AS oc', 'delivery_notes.hub_id', '=', 'oc.id')
        ...
        if ($rn_no = $request->get('search_dn_no')) {
            $deliveries->where('delivery_notes.id', '=', $rn_no);
        }
        if ($tracking = $request->get('search_tracking')) {
            $deliveries->join('delivery_note_shipments as rns', 'rns.delivery_note_id', '=', 'delivery_notes.id')
                ->join('shipments as s', 'rns.shipment_id', '=', 's.id')
                ->where('s.tracking_number', '=', $tracking);
        }
        if ($rider = $request->get('search_rider')) {
            $deliveries->where('riders.id', '=', $rider);
        }
        if ($rider = $request->get('rider_cnic')) {
            $deliveries->where('riders.id', '=', $rider);
        }
        if ($created_by = $request->get('search_assigned_by')) {
            $deliveries->where('admins.id', '=', $created_by);
        }
        if ($submitted_by = $request->get('search_updated_by')) {
            $deliveries->where('ub.id', '=', $submitted_by);
        }
        if ($hub = $request->get('search_hub')) {
            $deliveries->where('oc.id', '=', $hub);
        }
        if ($mode = $request->get('search_shipping_mode')) {
            $deliveries->where('shipments.booking_type_id', '=', $mode);
        }
        if ($courier_id = $request->get('courier_id')) {
            $deliveries->where('rider_categories.id', '=', $courier_id);
        }
        if ($submission_date = $request->get('search_submission')) {
            $deliveries->whereDate('delivery_notes.updated_at', $submission_date);
        }
        if ($request->get('search_date_from') && $request->get('search_date_to')) {
            $from = $request->get('search_date_from');
            $to = $request->get('search_date_to');
        ...
        if ($request->get('update_date_from') && $request->get('update_date_to')) {
            ...
            $deliveries->whereBetween('delivery_notes.status_updated_at', [$from, $to]);
        }
        ...
        $datatable = Datatables::of($deliveries)
            ...
            })->rawColumns(['delivered_shipments_link','shipments_count_link','delivery_note_link']);
    ...
    public function overall_sales_index()
    {
        ...
        $new_rider_referral_names = DB::connection('reports')->table('riders')
        ->join('sales_commission_users as scu', 'scu.user_id', '=', 'riders.id')
        ->where('scu.tier_id','=',4)
        ...
        ->union($new_rider_referral_names)
        ->get();
    ...
    public function overall_sales_list(Request $request)
    {
        ...
        if ($sub_segment = $request->get('sub_segment')) {
            $sales->where('u.sub_segment_id', '=', $sub_segment);
        }
        ...
        if ($search_origin_zone = $request->get('search_origin_zone')) {
            $sales->where('ocz.id', '=', $search_origin_zone);
        }
        if ($service_type_select = $request->get('service_type_select')) {
        ...
        if ($request->get('rider_type_referral') == 1) {
            $sales->where(['scu.tier_id' => $sales_tiers, 'scu.user_type' => 2]);
        }

        if ($request->get('rider_type_referral') == 2) {
            $sales->where(['scu.tier_id' => $sales_tiers, 'scu.user_type' => 1]);
        }

        if ($request->get('rider_type_referral') == 3) {
            $sales->WhereNotExists(function ($subquery) {
        ...
        $datatable = Datatables::of($sales)
            ...
            ->editColumn('fintech_charges', function ($shipments) {
                return number_format($shipments->fintech_charges, 2);
            })
            ...
            ->editColumn('reverse_pickup_charges', function ($shipment) {
                return number_format($shipment->reverse_pickup_charges, 2);
            })
            ...
            ->editColumn('ref', function ($users) {
                ...
                $shipper = DB::table('sales_commissions')->where('shipper_id', $users->account_no)->first();
    ...
    public function revenue_index()
    {
        ...
        return view('admin.reports.revenue')->with(['business_categories' => $business_categories, 'shippers' => $shippers, 'cities' => $cities, 'hubs' => $hubs, 'statuses' => $statuses, 'shipping_modes' => $shipping_modes, 'service_types'=> $service_types]);
    }
    ...
    public function revenue_list(Request $request)
    {
        ...
        $datatable = Datatables::of($sales)
            ...
            ->editColumn('faf_charges', function ($sale) {
    ...
    public function crm_list(Request $request)
    {
        try {
            ...
            $select = [
                'crm_requests.id as request_number_id',
                ...
                DB::raw("
                    CASE 
                        WHEN crm_requests.case_nature_complainant = 1 THEN 'Consignee' 
            ...
            Log::channel('code_test_log')->error(
                ...
                [
                    'trace' => json_encode($th->getTraceAsString()),
                    'line'  => $th->getLine()
                ]
            );
        }
    ...
    public function summary_index(Request $request)
    {
        ...
        return view('admin.reports.summary')->with(['hubs' => $hubs, 'shippers' => $shippers, 'today' => $today, 'thirtyday' => $thirtyDays, 'shipping_modes' => $shipping_modes, 'sub_segments' => $sub_segments, 'sales_persons' => $sales_persons, 'service_types'=> $service_types ]);
    }
    ...
    public function summary_list(Request $request)
    {
        ...
        if ($sales_persons = $request->get('search_sales_person')) {
            ...
            $shipments = $shipments->whereIn('st.admin_id', $sales_persons);
        }
    ...
    public function adjustments_list(Request $request)
    {
        ...
        $adjustments = DB::connection('reports')->table('adjustment_logs')
            ...
            ->select(
                'adjustment_logs.id as adjustment_id',
                'adjustment_logs.adjustment_amount as adjustment_amount',
                'adjustment_logs.remarks as remarks',
                's.tracking_number as tracking_number',
                'at.name as adjustment_type',
                'adjustment_logs.created_at as created_at',
                'a.name as created_by',
                'u.name as shipper_name',
                'dps.done_payment_id as done_payment_id',
                'oc.name as origin',
                'dc.name as destination',
                'u.id as shipper_id',
                'oc.id as origin_id',
    ...
    public function daily_visit_list(Request $request)
    {
        ...
        $daily_visit = DB::connection('reports')->table('daily_visits')
            ...
            ->select(
                'daily_visits.id as id',
                'a.name as admin',
                'daily_visits.admin_id as admin_id',
                'daily_visits.company_name as company_name',
                'daily_visits.customer_name as customer_name',
                'daily_visits.customer_address as customer_address',
                'daily_visits.phone_no as phone_no',
                'daily_visits.email as email',
                'dvls.name as lead_status',
                'daily_visits.feedback as feedback',
                'daily_visits.latitude as latitude',
                'daily_visits.longitude as longitude',
                'daily_visits.created_at as created_at',
                'daily_visits.business_card_image as business_card_image',
                'daily_visits.location_image as location_image',
                'c.name as city',
                'z.name as zone',
                'rate.name as rating_text',
                'daily_visits.comment as rating_comment',
                'rate.code as rating',
                'daily_visits.visit_status as visit_status'
            );
    ...
    public function route_distribution_index()
    {
        ...
        return view('admin.reports.route_distribution_summary_report')->with([
            'hubs' => $hubs, 'destination_cities' => $destination_cities,
    ...
    public function route_distribution_list(Request $request)
    {
        ...
        $datatables = Datatables::of($route_distribution_summary)
            ...
            ->addColumn('excel_ecom_cod', function($result){
                ...
                $count = DeliveryController::get_segment_type('delivery_note_shipments', $dn_ids, 2, 5, null,'delivery_note_id');
                return $count > 0 ? $count : '-';
            })
            ->addColumn('excel_general_retail', function($result){
                $count = 0;
                $dn_ids = explode(',' , $result->dn_ids);
                $count = DeliveryController::get_segment_type('delivery_note_shipments', $dn_ids, 1, 12, null, 'delivery_note_id');
                return $count > 0 ? $count : '-';
            })
            ->addColumn('excel_general_ecom_express', function($result){
                ...
                $count_ecomm = 0;
                $dn_ids = explode(',' , $result->dn_ids);
                $count_general = DeliveryController::get_segment_type('delivery_note_shipments', $dn_ids, 1, 2, null,'delivery_note_id');
                $count_ecomm = DeliveryController::get_segment_type('delivery_note_shipments', $dn_ids, 2, 7, null,'delivery_note_id');
    ...
    public function last_mile_app_list(Request $request)
    {
        ...
        $new_deliveries = RiderWiseDeliveryNoteSummary::join('riders as r', 'r.id', 'rider_wise_delivery_note_summaries.rider_id')
            ...
            ->select(
                'rider_wise_delivery_note_summaries.id as id',
                'rider_wise_delivery_note_summaries.rider_id',
                ...
                'rider_wise_delivery_note_summaries.delivery_date',
                'rider_wise_delivery_note_summaries.shipment_update_count as total_updated_shipments',
                'rider_wise_delivery_note_summaries.before_11_count',
                'rider_wise_delivery_note_summaries.at_11_count',
                'rider_wise_delivery_note_summaries.at_12_count',
                'rider_wise_delivery_note_summaries.at_13_count',
                'rider_wise_delivery_note_summaries.at_14_count',
                'rider_wise_delivery_note_summaries.at_15_count',
                'rider_wise_delivery_note_summaries.at_16_count',
                'rider_wise_delivery_note_summaries.at_17_count',
                'rider_wise_delivery_note_summaries.at_18_count',
                'rider_wise_delivery_note_summaries.at_19_count',
                'rider_wise_delivery_note_summaries.at_20_count',
                'rider_wise_delivery_note_summaries.at_21_count',
                'rider_wise_delivery_note_summaries.at_22_count',
                'rider_wise_delivery_note_summaries.at_23_count',
                'rider_wise_delivery_note_summaries.after_23_count',
                'z.name as zone',
                'h.name as hub',
    ...
    public function weight_qc_list(Request $request)
    {
        ...
        //        }
            ...
            ->select(['shipments.id as shId', 'shipments.tracking_number', 'u.name as shipper', 'oc.name as origin', 'dc.name as destination', 'shipments.created_at as booking_date', 'arv_date.created_at as arrival_date', 'sm.mode as shipping_mode', 'shipments.estimated_weight', 'shipments.actual_weight', 'shipments.length', 'shipments.breadth', 'shipments.height', 'scs.name as sub_segment', 'sw.weight_type', 'wt.name as weight_type_name','shipments.chargeable_weight', 'hub.name as hub_name','area.name as area_name', 'shipments.replacement_weight'
        ...
        if ($search_shipping_mode = $request->get('search_shipping_mode')) {
            $shipments->where('shipments.shipping_mode_id', $search_shipping_mode);
        }
        if ($tracking_numbers = $request->get('tracking_numbers')) {
            $shipments->whereIn('shipments.tracking_number', explode(',', $tracking_numbers));
        }
        ...
        if ($hub = $request->get('search_origin_hub')) {
            ...
            $shipments->whereIn('oc.hub_id', $hub);
        }
        ...
        if ($weight_type = $request->get('weight_type')) {
            $shipments->where('sw.weight_type', $weight_type);
        }
    ...
    public function rider_pickup_list(Request $request)
    {
        ...
        if ($request->get('search_from') && $request->get('search_to')) {
            $from = $request->get('search_from');
            $to = $request->get('search_to');
            $rider_pickup = $rider_pickup->whereBetween('v2_pickup_notes.created_at', [$from, $to]);
        }
    ...
    public function project_arrival_index()
    {
        ...
        return view('admin.reports.project_arrival_report')->with(['shippers' => $shippers]);
    }

    public static function project_arrival_list(Request $request)
    {
        if ($request->get('excel') && $request->get('excel') == true) {
    ...
    public function overland_list(Request $request)
    {
        ...
        $overland = DB::connection($connection)->table('shipments')->join('users as u', 'u.id', '=', 'shipments.user_id')
            ...
            ->select(
                ...
                'u.id as account_no',
                'u.name as shipper',
                'usi.pickup_address as shipper_address',
                'ss.name as current_status',
                'bt.booking_type as service_type',
                'shipments.created_at as booked_date',
                'sja.created_at as arrival_date',
                'sjb.created_at as intransit_date',
                'sjc.created_at as arrived_at_destination_date',
                'sjd.created_at as out_for_delivery_date',
                'sje.created_at as first_out_for_delivery_date',
                'oc.name as origin',
                'dc.name as destination',
                'h.name as hub',
    ...
    {   
        ...
        ->leftjoin('rv_shipment_assign_agent_details',function($join){
            ...
            ->whereRaw('TIMESTAMPDIFF(MINUTE, rv_shipment_assign_agent_details.created_at, rv_agent_call_histories.created_at) <= 10');
        })
        ->leftjoin('users', 'shipments.user_id', 'users.id')
        ->leftjoin('user_shipping_infos as uso', 'shipments.pickup_address_id', 'uso.id')
        ->leftjoin('cities as origin_city', 'uso.city_id', 'origin_city.id')
    ...
    {
        ActivityTrailController::createActivityTrailLog(Auth::id(), 806);
        ...
        return view('admin.reports.rvr_reattempt.index', [
            'shippers' => $shippers,
            'agents' => $agents,
        ]);
    }

    public function rvReattemptList(Request $request)
    {
        if ($request->get('excel') && $request->get('excel') == true) {
        ...
        ->leftJoin('shipments_journey as sjj_destination', function ($join) {
                ...
                'sjj_destination.id',
                        '=',
                        DB::connection('reports')->raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = sjj_destination.shipment_id and shipments_journey.shipper_status_id  = 4)')
                    );
        })
        ->leftJoin('shipments_journey as sj_destination', function ($join) {
                ...
                'sj_destination.id',
                        '=',
                        DB::connection('reports')->raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = sj_destination.shipment_id and shipments_journey.shipper_status_id  = 4)')
                    );
        })
        ->leftJoin('shipments_journey as sj_rvr', function ($join) {
                ...
                'sj_rvr.id',
                        '=',
                        DB::connection('reports')->raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = sj_rvr.shipment_id and shipments_journey.shipper_status_id  = 12)')
                    );
        })
    ...
    public function hbl_konnect_index()
    {

        ActivityTrailController::createActivityTrailLog(Auth::id(), 687);
        return view('admin.reports.hbl_connect_report');
    }


    public function hbl_konnect_list(Request $request)
    {

        if ($request->get('excel') && $request->get('excel') == true) {
            ActivityTrailController::createActivityTrailLog(Auth::id(), 688);
        }
        $hbl_konect_data = HblKonnectTransaction::select('transaction_id', 'delivery_note_id', 'amount', 'created_at');


        $datatable = Datatables::of($hbl_konect_data);
    ...
    public function pay_fast_report_index()
    {
        ActivityTrailController::createActivityTrailLog(Auth::id(), 689);

        return view('admin.reports.payfast_report');
    }


    public function pay_fast_report_list(Request $request)
    {
    ...
    public function fintech_report_index()
    {
        ActivityTrailController::createActivityTrailLog(Auth::id(), 691);

        $riders = Rider::select('id', 'name')->get();
        $shippers = User::get();
        $fintech_companies = FintechCompany::get();
        $hubs = DB::table('cities')->where('hub', 1)->select('id', 'name')->get();
        $fintech_transactions = FintechPaymentDetails::get();

        $delivery_notes = DeliveryNoteShipment::groupBy('delivery_note_id')->get();
        return view('admin.reports.fintech_report')->with(['riders' => $riders, 'shippers' => $shippers, 'fintech_companies' => $fintech_companies, 'hubs' => $hubs, 'fintech_transactions' => $fintech_transactions, 'delivery_notes' => $delivery_notes]);
    }

    public function fintech_report_list(Request $request)
    {
        if ($request->get('excel') && $request->get('excel') == true) {
            ActivityTrailController::createActivityTrailLog(Auth::id(), 692);
        }

        $fintech = DB::connection('reports')->table('shipments')->join('trax_pay_transactions', 'trax_pay_transactions.shipment_id', 'shipments.id')
            ->leftjoin('fintech_payment_details as fpd', 'fpd.trax_pay_id', 'trax_pay_transactions.id')
            ->leftjoin('users', 'users.id', 'shipments.user_id')
            ->leftjoin('riders', 'riders.id', 'fpd.rider_id')
            ->leftjoin('shipment_fintech_charges as sfc', 'sfc.shipment_id', 'trax_pay_transactions.shipment_id')
            ->leftjoin('fintech_companies as fc', 'fc.id', 'fpd.fintech_company_id')
            ->leftjoin('fintech_company_charges as fcc', 'fc.id', 'fcc.company_Id')
            ->leftjoin('shipments_journey as sj', function ($join) {
                $join->on('sj.shipment_id', '=', 'trax_pay_transactions.shipment_id')
                    ->where('sj.id', '=', DB::raw('(select max(id) from shipments_journey where shipments_journey.shipment_id = trax_pay_transactions.shipment_id)'));
            })
        ...
        $datatables = Datatables::of($fintech)
            ->addColumn('fed_tax', function ($result) {
                $standard_fintech_charges = DB::table('standard_fintech_charges')->first();
                $shipment = Shipment::where('tracking_number', $result->tracking_number)->first();
                $fintech_charges = DB::table('shipment_fintech_charges')->where('shipment_id', $shipment->id)->first();
                if ($fintech_charges) {
                    $fintech_charges = intval($fintech_charges->fintech_charges);
                    $calculation = $result->cod_amount * ($standard_fintech_charges->standard_fintech_charges / 100) - $fintech_charges;
                    $result->fed_tax_calculation = $calculation;
                    return $calculation;
                } else {
                    return '---';
                }
            })
            ->editColumn('tracking_number', function ($result) {
                $route = route('admin.tracking.index');
                $trackingLink = "$route?tracking_number=$result->tracking_number";
                return [
    ...
    public function ordinary_discrepancy_report_list(Request $request)
    {
        ...
        $tracking_data = OrdinaryDiscrepancyReport::join('shipments', 'shipments.id', 'ordinary_discrepancy_reports.shipment_id')
            ...
            ->select(
                ...
                'shipments.pieces as quantity_by_shipper',
                'shipments.amount as cod_value',
                'si.description as shipment_content_by_shipper',
                'ordinary_discrepancy_reports.product_content as shipment_content_by_admin',
    ...
    public function ordinary_discrepancy_report_tracking_data(Request $request)
    {
        ...
        $tracking_data = Shipment::join('users', 'users.id', 'shipments.user_id')
            ...
            ->select(
                ...
                'shipments.amount as cod_value',
                'users.name as shipper_name',
                'si.description as shipment_content_shipper',
                'ss.id as shipment_status_id'
            )
...
//            }, []);
        ...
        public function ibft_report_list(Request $request)
    {
        ...
        $ibft_report = DB::connection('reports')->table('done_payment_calculations')->join('done_payments as dp', 'dp.id', '=', 'done_payment_calculations.done_payment_id')
            ...
            ->select(
                'dp.id as done_payment_id',
                'u.name as shipper',
                'done_payment_calculations.amount',
                'done_payment_calculations.charges',
                'done_payment_calculations.gst',
                'done_payment_calculations.ibft_charges',
                'done_payment_calculations.payable',
                'done_payment_calculations.created_at',
    ...
    public function fetch_rv_action_count_report(Request $request)
    {
        if ($request->get('search_date_from') && $request->get('search_date_to')) {
            $from = $request->get('search_date_from');
            $to = $request->get('search_date_to');
            ...
            return response()->json(['failure' => 0, 'error' => 'No Date Found!']);
        }
    }
	public function sack_bag_utilization_index()
    {
        ...
        ActivityTrailController::createActivityTrailLog(Auth::id(), 741);
    ...
    {
        ActivityTrailController::createActivityTrailLog(Auth::id(), 745);
        $origins = City::where('status', '=', 1)->get();
    ...
    public function issuance_sack_bag_list(Request $request)
    {
        ...
        if ($request->get('excel') && $request->get('excel') == true) {
            ActivityTrailController::createActivityTrailLog(Auth::id(), 746);
        }
    ...
    public function sack_bag_status_index()
    {
        ActivityTrailController::createActivityTrailLog(Auth::id(), 743);
        $destinations = City::where('status', '=', 1)->get();
        return view('admin.reports.sack_bag_status_report', compact('destinations'));
    }
    public function sack_bag_status_list(Request $request)
    {
        ...
        $sack_bag_statuses = City::
            ...
            ->leftJoin('issue_sack_bag_origins as tm', function ($join) {
                $join->on('cities.id', '=', 'tm.sack_destination_id')
                    ->where('tm.sack_status_id', '=', 3);
            })
            ->leftJoin('issue_sack_bag_origins as br', function ($join) {
                $join->on('cities.id', '=', 'br.sack_destination_id')
                    ->where('br.sack_status_id', '=', 4);
            })
            ->leftJoin('issue_sack_bag_origins as sdm', function ($join) {
                $join->on('cities.id', '=', 'sdm.sack_destination_id')
                    ->where('sdm.sack_status_id', '=', 5);
            })
            ->groupBy('cities.name')
    ...
    public function fetchCsv($headers ,$fieldsToRetrieve, $shipments,$select)
    {
            ...
            foreach ($specificValues as $key => $row) {
                ...
                //Total Attempt before formatting tracking_number
                $shipment = Shipment::where('tracking_number', $rowArray['tracking_number'])->first()->id ?? 0;
                $delivery_note_shipment = DeliveryNoteShipment::where('shipment_id', $shipment);
                ...
                if ($delivery_note_shipment->exists()) {
                    ...
                    $rowArray['total_attempt'] = $delivery_notes;
                } else {
                    $rowArray['total_attempt'] = '-';
                }
    ...
    public function kam_and_poc_qsr_index() {
        ...
        $table_headers = [
            ...
            "Case Nature",
            "Case Nature Type",
            "Complaint Launched Date",
            "TAT (launced + in process)",
            "Closed Date",
            "Case Closed Remark",
            "Complainant",
    ...
    public function kam_and_poc_qsr_list(Request $request) {
        ...
        $select = [
            'shipments.id as shipment_id',
            'shipments.tracking_number as shipment_tracking_number',  // Renamed tracking_number
            'shipments.tracking_number as tracking_number_link',      // Intentional duplication for link, ensure it's needed
            'shipments.order_id as shipment_order_id',
            'users.name as shipper',
            'admin_sale_person.id as admin_sale_person_id',  // Unique alias for admin_sale_person
            'admin_sale_person.name as sale_person_name',
            'shipments_journey_first_attempt.created_at as first_attempt_date',
            'shipments_journey_rider_picked.created_at as rider_picked_status_date',
            'shipment_status.name as status_name',
            ...
            'cargo_bags.seal_number as bag_seal_number',
            'shipment_arival_journey.created_at as origin_arrival_date',
            'destination_shipment_journey.created_at as destination_arrival_date',
            'journey.created_at as last_status_date',
            'shipments.created_at as booked_date',
            'shipping_mode.mode as shipping_mode',
            'origin_city.name as origin',
            'destination_city.name as destination',
            'hub.name as hub',
            'city_area.name as area',
            'concerned_hub.name as concerned_hub_name',  // Fixed typo: "concerened_hub" to "concerned_hub"
            'return_city.name as return_city',
            'zone.name as zone',
            'shipments.consignee_name as consignee_name',
            'shipments.consignee_phone_number_1 as consignee_number',
            'shipments.consignee_address as consignee_address',
            'product.product_name as product_type',
            'shipments.amount as amount',
            ...
            'riders.name as rider_name',
            'crm_case_nature_types.type as crm_case_nature_type',
            'status_name.name as request_status',
        ...
        $shipments = DB::connection($connection)
            ...
            ->leftJoin('shipments_journey as shipments_journey_rider_picked', function ($join) use ($connection) {
                $join->on('shipments_journey_rider_picked.shipment_id', '=', 'shipments.id')
                    ->where('shipments_journey_rider_picked.id', '=', DB::connection($connection)->raw('(
                        select min(id) 
                        from shipments_journey 
                        where shipments_journey.shipment_id = shipments.id 
            ...
            ->leftJoin('admins as admins', function ($join) {
                $join->on('admins.id', '=', 'ssj_last_location.admin_id')
            ...
            ->leftJoin('city_areas as city_area_last_scan_location', 'city_area_last_scan_location.id', '=', 'ssjal_last_location.area_id')
            ->leftJoin('shipment_status_reason', 'shipment_status_reason.id', '=', 'journey.status_reason_id')

            ->leftJoin('shipments_journey as shipments_journey_remarks', function ($join) use ($connection) {
                $join->on('shipments_journey_remarks.shipment_id', '=', 'shipments.id')
                    ->where('shipments_journey_remarks.id', '=', DB::connection($connection)->raw('(
                        select max(id) 
                        from shipments_journey 
                        where shipments_journey.shipment_id = shipments.id 
            ...
            ->leftJoin('shipment_scanning_journeys as shipment_journey_last_scanned_by', function ($join) {
                $join->on('shipment_journey_last_scanned_by.shipment_id', '=', 'journey.shipment_id')
                    ->whereRaw('shipment_journey_last_scanned_by.id = (
                        select max(id) 
                        from shipment_scanning_journeys 
                        where shipment_scanning_journeys.shipment_id = journey.shipment_id 
                        and 
                        shipment_scanning_journeys.screen_location_id not in (9, 18)
                    )');
            })
            ...
            ->leftJoin('cargo_manifest_bag_shipments as cargo_bag_shipments', function ($join) {
                $join->on('cargo_bag_shipments.shipment_id', '=', 'shipments.id')
                    ->where(
            ...
            ->leftjoin('cargo_manifest_bags as cargo_bags', 'cargo_bags.id', '=', 'cargo_bag_shipments.cargo_manifest_bag_id')
            ->leftjoin('cargo_manifest_bag_statuses as cargo_status', 'cargo_status.id', '=', 'cargo_bags.status_id')
            ->leftjoin('bag_statuses as bag_status', 'bag_status.id', '=', 'cargo_bags.status_id')
            ->leftJoin('shipments_journey as destination_shipment_journey', function ($join) {
                $join->on('destination_shipment_journey.shipment_id', '=', 'shipments.id')
                        ->where(function ($query) {
                            // First check if status 4 (Arrived at Destination)
                            $query->where('destination_shipment_journey.shipper_status_id', '=', 4)
                                ->orWhere(function ($query) {
                                        // If not, check status 2 (Arrived at Origin)
                                        $query->where('destination_shipment_journey.shipper_status_id', '=', 2)
                                            ->whereNotExists(function ($query) {
                                                // Check if a status 4 record exists for this shipment_id
                                                $query->select(DB::raw(1))
                                                    ->from('shipments_journey')
            ...
            ->leftjoin('user_shipping_infos AS return_shipping_info', 'shipments.return_address_id', '=', 'return_shipping_info.id')
            ->leftjoin('cities AS return_city', 'return_shipping_info.city_id', '=', 'return_city.id')
            ->leftjoin('zones as zone', 'zone.id', '=', 'hub.zone_id')
            ->leftjoin('shipment_items as shipment_item', function ($join) {
                $join->on('shipment_item.shipment_id', '=', 'shipments.id')
                    ->where('shipment_item.type', '=', 0);
            })
            ->leftjoin('products as product', 'product.id', '=', 'shipment_item.product_type_id')
            ->leftJoin('crm_requests as request_number', function ($join) {
                $join->on('request_number.shipment_id', '=', 'shipments.id')
                    ->where(
                        'request_number.id',
                        '=',
                        DB::connection('reports')->raw('(select max(id) from crm_requests where crm_requests.shipment_id = shipments.id)')
            ...
            ->leftJoin(DB::raw('(
                SELECT history.crm_request_id, history.status_id
                FROM crm_request_status_histories AS history
                INNER JOIN (
                    SELECT crm_request_id, MAX(id) AS latest_history_id
                    FROM crm_request_status_histories
                    GROUP BY crm_request_id
                ) AS latest ON history.id = latest.latest_history_id
            ) AS latest_status_history'), 'latest_status_history.crm_request_id', '=', 'request_number.id')
            ->leftJoin('crm_request_statuses AS status_name', 'status_name.id', '=', 'latest_status_history.status_id')
            
            ->leftJoin('crm_request_status_histories as crm_valid_status', function ($join) {
                $join->on('crm_valid_status.crm_request_id', '=', 'request_number.id')
                    ->whereIn('crm_valid_status.status_id', [6, 7])
                    ->whereRaw('crm_valid_status.id = (
                        select max(id)
                        from crm_request_status_histories
                        where crm_request_status_histories.crm_request_id = request_number.id
                        and crm_request_status_histories.status_id in (6, 7)
                    )');
            })
            ->leftJoin('crm_request_statuses as crm_valid_status_name', 'crm_valid_status.status_id', '=', 'crm_valid_status_name.id')
                ...
                $join->on('request_in_process_created_at.shipment_id', '=', 'shipments.id')
                    ->where(
                        ...
                        DB::connection('reports')->raw('(
                            ...
                            from crm_requests 
                            where crm_requests.shipment_id = shipments.id 
                            and crm_requests.status_id = 2
                        )')
                    );
            })
            ->leftJoin('crm_requests as request_closed_created_at', function ($join) {
                $join->on('request_closed_created_at.shipment_id', '=', 'shipments.id')
                    ->where(
                        ...
                        DB::connection('reports')->raw('(
                            ...
                            from crm_requests 
                            where crm_requests.shipment_id = shipments.id 
                            and crm_requests.status_id = 4
                        )')
                    );
            })
            ->leftJoin('crm_requests as case_closed_requests', function ($join) {
                $join->on('case_closed_requests.shipment_id', '=', 'shipments.id')
                    ->where('case_closed_requests.status_id', '=', 4) // status_id = 4 for closed
                    ->whereRaw('case_closed_requests.id = (
                        select max(id) 
                        from crm_requests 
            ...
            ->leftJoin('crm_comments', function ($join) {
                $join->on('crm_comments.crm_request_id', '=', 'case_closed_requests.id')
                    ->whereRaw('crm_comments.id = (
                        select max(id) 
                        from crm_comments 
        ...
        ->addColumn('aging', function ($shipment) {
            $days = Carbon::now()->diffInDays($shipment->origin_arrival_date);
            if ($days == 0) {
                return "-";
            } else {
                return $days;
        ...
        ->addColumn('aging_last_status', function ($shipment) {
        ...
        ->addColumn('current_tat', function ($shipment) {
            // Launched TAT
            $launched_tat = 0;
            if ($shipment->request_launched_date_created_at) {
                Carbon::setWeekendDays([
                    Carbon::SUNDAY,
                ]);
                $re_open_count = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)
                ->where('status_id' ,5)
                ->latest('id')
                ->first();
                ...
                if ($re_open_count) {
                    ...
                    $last_closed = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)
                    ->where('status_id' ,4)
                    ->where('created_at', '>=', $re_open_count->created_at)
                    ->first();
                    ...
                    $to_formatted = Carbon::parse($time_to->setting_value)->format($time_format);

                    $launched = Carbon::parse($shipment->request_launched_date_created_at);
                    ...
                    $launched = Carbon::parse($shipment->request_launched_date_created_at)->startOfDay();
                    $first_closed = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)
                    ->where('status_id', 4)
                    ->first();
                    if($first_closed){
                ...
                $launched_tat = $current_tat;
            }

            // In process TAT
            $in_process_tat = 0;
            if ($shipment->request_in_process_created_at) {
                Carbon::setWeekendDays([
                    Carbon::SUNDAY,
                ]);
                $re_open_count = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)
                ...
                if ($re_open_count){
                    $last_closed = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)
                    ->where('status_id', 4)
                    ->where('created_at', '>=', $re_open_count->created_at)->first();
                    ...
                    $holidays = CrmTatHolidays::whereBetween('holiday', [$last_closed, $current])->get();
                    foreach($holidays as $holiday){
                        $holiday_formatted = date('Y-m-d H:i:s', strtotime($holiday->holiday));
                        $holiday_formatted_check = date('Y-m-d', strtotime($holiday->holiday));
                        $last_closed_formatted_check = date('Y-m-d', strtotime($last_closed));
                        if($last_closed < $holiday_formatted || $current > $holiday_formatted){
                            if($holiday_formatted_check == $last_closed_formatted_check){
                                if($to_formatted < $cut_off_check){
                                    $after_cut_off = $current_tat + 1;
                                    $current_tat = $after_cut_off;
                                }
                    ...
                    $last_closed = Carbon::parse($shipment->request_in_process_created_at)->startOfDay();
                    $first_closed = CrmRequestStatusHistory::where('crm_request_id', $shipment->request_number_id)->where('status_id' ,4)->first();
                    if($first_closed){
                        $current = $first_closed->created_at;
                    }
                    else{
                    ...
                    $cut_off_check = \Carbon\Carbon::parse($shipment->request_in_process_created_at)->format($time_format);
                    
                    $current_tat = $current->diffInWeekdays($last_closed);
        ...
        ->editColumn('consignee_number', function ($shipment) {
            $consignee_phone = '-';
            if ($shipment->consignee_number != null) {
                $consignee_phone = $shipment->consignee_number;
            }
            return $consignee_phone;
        })
        ->editColumn('rider_name', function ($shipment) {
            $rider_name = '-';
            if ($shipment->rider_name != null) {
        ...
        ->editColumn('valid_invalid_crm_status', function ($shipment) {
        ...
        ->editColumn('crm_case_nature_type', function ($shipment) {
            if ($shipment->crm_case_nature_type) {
                return $shipment->crm_case_nature_type;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('request_status', function ($shipment) {
            if ($shipment->request_status) {
                return $shipment->request_status;
            } else {
                return '-';
            }
        })

        ->editColumn('sale_person_name', function ($shipment) {
            if ($shipment->sale_person_name) {
                return $shipment->sale_person_name;
            } else {
                return '-';
            }
        })

        ->editColumn('rider_picked_status_date', function ($shipment) {
            if ($shipment->rider_picked_status_date) {
                return $shipment->rider_picked_status_date;
            } else {
                return '-';
            }
        })

        ->editColumn('sub_hub', function ($shipment) {
            if ($shipment->sub_hub) {
                return $shipment->sub_hub;
            } else {
                return '-';
            }
        })

        ->editColumn('reason', function ($shipment) {
            if ($shipment->reason) {
        ...
        ->editColumn('first_attempt_date', function ($shipment) {
            if ($shipment->first_attempt_date) {
                return $shipment->first_attempt_date;
            } else {
                return '-';
            }
        })

        ->editColumn('shipment_journey_remarks', function ($shipment) {
            if ($shipment->shipment_journey_remarks) {
                return $shipment->shipment_journey_remarks;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('last_scanned_by', function ($shipment) {
            if ($shipment->last_scanned_by) {
                return $shipment->last_scanned_by;
            } else {
                return '-';
            }
        })

        ->editColumn('last_scanned_by_date', function ($shipment) {
            if ($shipment->last_scanned_by_date) {
                return $shipment->last_scanned_by_date;
            } else {
                return '-';
            }
        })

        ->editColumn('last_location_screen_location_name', function ($shipment) {
            if ($shipment->last_location_screen_location_name) {
                return $shipment->last_location_screen_location_name;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('bag_seal_number', function ($shipment) {
            if ($shipment->bag_seal_number) {
                return $shipment->bag_seal_number;
            } else {
                return '-';
            }
        })

        ->editColumn('bag_status', function ($shipment) {
            if ($shipment->bag_status) {
                return $shipment->bag_status;
            } else {
                return '-';
            }
        })

        ->editColumn('origin_arrival_date', function ($shipment) {
            if ($shipment->origin_arrival_date) {
                return $shipment->origin_arrival_date;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('destination_arrival_date', function ($shipment) {
            if ($shipment->destination_arrival_date) {
                return $shipment->destination_arrival_date;
            } else {
                return '-';
            }
        ...
        ->editColumn('area', function ($shipment) {
            if ($shipment->area) {
                return $shipment->area;
            } else {
                return '-';
            }
        })

        ->editColumn('concerned_hub_name', function ($shipment) {
            if ($shipment->concerned_hub_name) {
                return $shipment->concerned_hub_name;
            } else {
                return '-';
            }
        })

        ->editColumn('return_city', function ($shipment) {
            if ($shipment->return_city) {
                return $shipment->return_city;
            } else {
                return '-';
            }
        ...
        ->editColumn('valid_invalid_crm_status', function ($shipment) {
            if ($shipment->valid_invalid_crm_status == 6) {
                return "Valid";
            } else if ($shipment->valid_invalid_crm_status == 7) {
                return "Invalid";
            } else {
                return '-';
            }
        ...
        ->editColumn('request_launched_date_created_at', function ($shipment) {
            if ($shipment->request_launched_date_created_at) {
                return $shipment->request_launched_date_created_at;
            } else {
                return '-';
            }
        })

        ->editColumn('request_closed_created_at', function ($shipment) {
            if ($shipment->request_closed_created_at) {
                return $shipment->request_closed_created_at;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('complainant_phone_number', function ($shipment) {
            if ($shipment->complainant_phone_number) {
                return $shipment->complainant_phone_number;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('rider_name', function ($shipment) {
            if ($shipment->rider_name) {
                return $shipment->rider_name;
            } else {
                return '-';
            }
        })

        ->editColumn('shipment_order_id', function ($shipment) {
            if ($shipment->shipment_order_id) {
                return $shipment->shipment_order_id;
            } else {
                return '-';
            }
        })
        ...
        ->editColumn('tracking_number_link', function ($shipment) {
            $route = route('admin.tracking.index');
            return "<u><a href='{$route}?tracking_number=$shipment->shipment_tracking_number' class='tracking' target='_blank'>$shipment->shipment_tracking_number</a></u>";
        })
        ...
        if ($request->get('excel') && $request->get('excel') == true) {
            ...
            $this->kam_poc_qsr_csv_export($headers,$fieldsToRetrieve,$shipments,$select);
            ActivityTrailController::createActivityTrailLog(Auth::id(), 138);
        }else{
            return $datatable->make(true);
        }
    ...
    public function kam_poc_qsr_csv_export($headers ,$fieldsToRetrieve, $shipments,$select)
    {
        $headers = array_filter($headers, function($value) {
            return $value !== 'Select All';
        });            
        ...
        foreach ($specificValues as $key => $row) {
            ...
            //Total Attempt before formatting tracking_number
            $shipment = Shipment::where('tracking_number', $rowArray['shipment_tracking_number'])->first()->id ?? 0;
            $delivery_note_shipment = DeliveryNoteShipment::where('shipment_id', $shipment);
            ...
            $days = Carbon::now()->diffInDays($rowArray['origin_arrival_date']);
            $rowArray['aging'] = ($days == 0) ? "-" : $days;
            $days = Carbon::now()->diffInDays($rowArray['last_status_date']);
            $rowArray['aging_last_status'] = ($days == 0) ? "-" : $days;

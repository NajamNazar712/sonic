...
//            ->join('user_bank_infos as ubi','ubi.user_id','=','u.id')
            ...
            ->leftJoin('shipments_journey as ffstatus', function ($join) {
                $join->on('ffstatus.shipment_id', '=', 'shipments.id')
                    ->where('ffstatus.id','>',
            ...
            ->leftJoin('shipment_status as fss','fss.id','=','ffstatus.shipper_status_id')
            ...
            ->leftjoin('shipments_journey as fsj', function($join){
                $join->on('fsj.shipment_id','=','shipments.id')
                    ->where('fsj.created_at','=',
                        DB::raw('(select min(created_at) from shipments_journey where shipments_journey.shipment_id = shipments.id)')
                    );
            })
            ->leftjoin('shipments_journey as lsj', function($join){
                $join->on('lsj.shipment_id','=','shipments.id')
                    ->where('lsj.created_at','=',
                        DB::raw('(select max(created_at) from shipments_journey where shipments_journey.shipment_id = shipments.id)')
                    );
            })
...
//            ->leftjoin('transport_mode_vendor as tmv','tmv.id','=','cc.transport_mode_vendor_id')
        ...
        $lead_time = Datatables::of($shipments)
            ...
            ->editcolumn('first_verification', function ($shipments) {
                if($shipments->first_verification == 1)
                {
                    return "Verified";
                }
                else
            ...
            ->editcolumn('last_verification', function ($shipments) {
                if($shipments->last_verification == 1)
                {
                    return "Verified";
                }
                else
...
//        return $shipments;
    ...
    public function daily_pickup_sales_export_to_excel(Request $request){
        ...
        if($search_city != null){
            ...
            $search_city_hub = $search_city_hub->hub_id;
        }else{
            $search_city_hub = '';
        }
            ...
            if (session('department_id') != 7){
                ...
                if(session('role_id') != 4){
                    ...
                    if($request->sales_person != null){
                        ...
                        $booked = Shipment::whereHas('pickup_address.city', function($query) use ($hub) {
                            $query->where('hub_id', '=', $hub->id);
                        })->whereDate('created_at',$date)->whereIn('user_id', $tagged_shippers)->count();
        ...
        if($search_city != null){
            $pickup_request_shippers = PickupRequest::whereHas('pickup_address.city', function($query) use($search_city_hub) {
                $query->where('hub_id', '=', $search_city_hub);
            })->whereDate('created_at', $date)->select('shipper_id')->get();
            if(session('department_id') != 7){
            ...
            if(session('department_id') != 7){
                $pickup_request_shippers = PickupRequest::whereDate('created_at', $date)->select('shipper_id')->get();
                $shippers = User::select('id','name')->whereIn('id', $pickup_request_shippers)->where('status',3)->get();
            }else{
                if(session('role_id') != 4){
                    $pickup_request_shippers = PickupRequest::whereDate('created_at', $date)->select('shipper_id')->get();
...
//            $sheet->setCellValue($dateIndex.$col,$shippers['total'][$id]);
    ...
    public function overall_sales_list(Request $request){
        $sales = Shipment::join('users as u','u.id','=','shipments.user_id')
            ...
            ->join('cities AS dc', 'shipments.consignee_city_id', '=', 'dc.id')
            ->join('cities as h' ,'dc.hub_id', '=' , 'h.id')
            ->join('station_deposit_notes as sdn', 'dc.hub_id', '=', 'sdn.hub_id')
            ->join('done_payments as dp', 'u.id', '=', 'dp.user_id')

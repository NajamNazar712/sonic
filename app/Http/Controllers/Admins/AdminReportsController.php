            ...
            ->leftJoin('shipments_journey_archive as dr', function ($join) use ($connection) {
                $join->on('dr.shipment_id', '=', 'shipments.id')
                    ...
                    ->where(
                        'dr.id',
                        '=',
                        DB::connection($connection)->raw('(select max(id) from shipments_journey_archive where shipments_journey.shipment_id = shipments.id and shipments_journey_archive.shipper_status_id In(14,25,30,36,37) and shipments_journey_archive.verification = 1)')
                    );
            })

...
//            }
    ...
    static public function auto_disable_days(Request $request)
        {
            ...
            if($user){
                if($days > 0 && $days != null){
                    $user->auto_account_disabled_days = $days;
                    $user->save();
                    return response()->json(['status' => 1, 'success' => 'Auto disable days updated successfully for ' . $user->name]);
                }
                else{

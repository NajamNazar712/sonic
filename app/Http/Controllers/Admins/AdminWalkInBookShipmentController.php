...
//    }
    ...
    public function international_book_index()
    {
        ...
        $user_shipping_infos = UserShippingInfo::where('user_id', $user_id)->get();
        $cities = WalkInCities::join('cities as c', 'c.id', '=', 'walk_in_cities.city_id')
            ->select('c.name as city_name', 'c.id as city_id')->where('walk_in_cities.pickup', 1)->where('cities.status', 1)->where('cities.hub', 1)->where('cities.business_category_id', 2)->whereNotNull('cities.zone_id')->get();

...
use App\Http\Models\HR\PenaltyStatus;
...
class AdminHumanResourseController extends Controller
{
    ...
    public function employee_directory_list(Request $request)
    {
        ...
        $employees = Employee::join('cities', 'employees.city_id', '=', 'cities.id')
            ...
            'employees.request_status_id', 'employees.status_id as status_id', 'employees.employee_type_id', 'eg.name as gender', 'employees.cnic', 'employees.phone_number', 
            'et.name as employee_type', 'ers.name as request_status', 'es.name as status', 'employees.created_at as requested_at', 'employees.pin as pin', 'employees.address as address', 
            'employees.guardian_name as father_name', 'ads.name as department_name','employees.shift_id as shift_id','employees.first_inactive', 'employees.rider_sub_category as rider_sub_category', 
            'employees.rider_main_category as rider_main_category_id','er_rt.name as rider_type','est.name as staff_category','employees.staff_category_id','employees.joining_date',
        ...
        $datatable = Datatables::of($employees)
            ...
            ->addColumn('shift', function ($user) {
                if($user->start_time != null && $user->end_time != null){
                    return $user->start_time . ' - ' . $user->end_time;
                }
                else{
                    return '-';
                }
            ...
            ->editColumn('sub_department_name', function($employee){
                if($employee->sub_department_name == null){
                    return '-';
                }
                else{
                    return $employee->sub_department_name;
                }
            })
...
//            $admins = Admin::where('designation_id',$designation->id)->get();
    ...
    public function payslip_list(Request $request)
    {
        ...
        $datatable = Datatables::of($payslips)
            ...
            ';
                if($payslip->pdf_id){
                    $dropdown .= '<button type="button" class="dropdown-item download" ><div class="row no-gutters align-items-center"><div class="col-2"><i class="ft-edit"></i></div><div class="col-9 offset-1">Download</div></button>';
                }
                else{
    ...
    public function payslip_excel_upload(Request $request)
    {
        ...
        $rules = [
            ...
            'return_incentive' => ['nullable', 'integer'],
            'attendance_allowance' => ['nullable', 'integer'],
        ...
        $fields = [0 => 'trax_id', 1 => 'name', 2 => 'designation', 3 => 'department', 4 => 'hub', 5 => 'zone', 6 => 'joining_date', 7 => 'cnic', 8 => 'employee_status', 9 => 'payroll_days', 10 => 'present_days', 11 => 'pay_cut_days', 12 => 'absent_days', 13 => 'extra_paid_days', 14 => 'fuel_days', 15 => 'basic_salary', 16 => 'house_rent', 17 => 'medical', 18 => 'gross_salary', 19 => 'mobile_allowance', 20 => 'vehicle_allowance', 21 => 'fuel_allowance', 22 => 'conveyance_allowance', 23 => 'vehicle_maintenance', 24 => 'fixed_incentive', 25 => 'holiday_allowance', 26 => 'overtime', 27 => 'bonus', 28 => 'arrears', 29 => 'pickup_incentive', 30 => 'delivery_incentive', 31 => 'operation_incentive',32 =>'retail_incentive',33 => 'sales_incentive',34 => 'return_incentive',35 => 'attendance_allowance',36 => 'extra_duty_allowance', 37 => 'others_addition', 38 => 'total_salary', 39 => 'paycut', 40 => 'absent', 41 => 'late_deduction', 42 => 'income_tax', 43 => 'eobi', 44 => 'advance_salary', 45 => 'month_closing', 46 => 'loan', 47 => 'fuel_card', 48 => 'open_parcel', 49 => 'phone_call', 50 => 'recovery', 51 => 'auction_sale', 52 => 'penalty', 53 => 'others_deduction', 54 => 'van_deduction', 55 => 'medical_insurance', 56 => 'total_deduction', 57 => 'net_salary', 58 => 'iban', 59 => 'confirmation_date', 60 => 'employee_type'];
        if ($file = $request->file('payslip')) {
            $spreadsheet = IOFactory::createReaderForFile($file);
            $spreadsheet->setReadDataOnly(true);
            $spreadsheet = $spreadsheet->load($file)->getActiveSheet()->toArray();

            $header = ['Employee ID', 'Employee Name', 'Designation', 'Department', 'Hub', 'Zone', 'Date of Joining', 'CNIC', 'Employee Status', 'Payroll Days', 'Present Days', 'Pay Cut Days', 'Absent Days', 'Extra Paid Days', 'Fuel Days', 'Basic Salary', 'House Rent', 'Medical', 'Gross Salary', 'Mobile Allowance', 'Vehicle Allowance', 'Fuel Allowance', 'Conveyance Allowance', 'Vehicle Maintenance', 'Fixed Incentive', 'Sunday / Holiday Allowance', 'Overtime', 'Bonus', 'Arrears', 'Pickup Incentive', 'Delivery Incentive', 'Operations Incentive','Retail Incentive','Sales Incentive','Return Incentive','Attendance Allowance','Extra Duty Allowance', 'Others Addition', 'Total Salary', 'Pay Cut', 'Absent', 'Late Deduction', 'Income Tax', 'EOBI', 'Advance Salary', 'Month Closing', 'Loan', 'Fuel Card', 'Open Parcel', 'Phone Call', 'Recovery', 'Auction Sale', 'Penalty', 'Others Deduction', 'Van Deduction', 'Medical Insurance', 'Total Deduction', 'Net Salary', 'IBAN', 'Confirmation Date', 'Employee Type'];
            ...
            if (!empty($spreadsheet) || !isset($spreadsheet)) {
                ...
                if (empty($errors)) {
                    ...
                    foreach ($rows as $key => $row) {
                        ...
                        $payslip_details['return_incentive'] = trim($row['return_incentive']);
                        $payslip_details['attendance_allowance'] = trim($row['attendance_allowance']);
                        ...
                        $payslip->return_incentive = trim($row['return_incentive']);
                        $payslip->attendance_allowance = trim($row['attendance_allowance']);
                        $payslip->save();
    ...
    public function payslip_print(Request $request)
    {
        ...
        if (Admin::where('trax_id', $payslip->trax_id)->exists()) {
            ...
            $return_incentive = ($payslip->return_incentive != null) ? number_format($payslip->return_incentive) : '-';
            $attendance_allowance = ($payslip->attendance_allowance != null) ? number_format($payslip->attendance_allowance) : '-';
            ...
            $html .= '<table class="table border table-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="text-left">
                                <td colspan="2" class="border twice-right">Return Incentive</td>
                                <td colspan="2"  class="border twice-right">'. $return_incentive .'</td>
                                <td colspan="2"  class="border twice-right">Van Deduction</td>
                                <td colspan="2"  class="border twice-right">' . $van_deduction . '</td>
                            </tr>
                            <tr class="text-left">
                                <td colspan="2" class="border twice-right">Attendance Allowance</td>
                                <td colspan="2"  class="border twice-right">'. $attendance_allowance .'</td>
    ...
    {  
        ...
        if($payslip){
            ...
            if ($payslip_pdf->exists()) {
                $month = Carbon::parse($payslip->payroll_month);
    ...
    public function employee_penalty_index()
    {
        ...
        $users = Admin::join('employees as e','e.trax_id','admins.trax_id')
            ...
            ->select('e.id', 'e.name','e.trax_id as trax_id','c.name as city_name')->get();
        
        $trax_ids = Admin::wherenotnull('trax_id')->pluck('trax_id')->toArray();
        $employee_confirmation_statuses = EmployeeConfirmationStatus::all();
        $penalty_statuses = PenaltyStatus::all();
        return view('admin.human_resource.employee_penalty')->with(["admins" => $users, "trax_ids" => $trax_ids, "employee_confirmation_statuses" => $employee_confirmation_statuses,"penalty_statuses" => $penalty_statuses]);
    }
    ...
    public function leave_index()
    {
        ...
        $remaing_leaves = max(0,$remaing_leaves);
...
//        }
    ...
    public function rider_fuel_allocation_list(Request $request){
        ...
        if ($search_rider = $request->get('search_rider')) {
            $fuel_allocation->where('rider_fuel_allocations.rider_id', $search_rider);
        }

        $datatable = Datatables::of($fuel_allocation)
            ->editColumn('delivery_notes_button', function ($fuel) {
                if ($fuel->delivery_notes > 0) {
                    return '<div class="text-center"><button type="button" class="btn btn-sm btn-outline-info delivery_notes_info">' . $fuel->delivery_notes . '</button></div>';
                } else {
                    return '-';
                }
                ...
                if (session('role_id') == 1 || in_array(832, session('permissions'))) {
                  ...
                  ';
                    ...
                    $dropdown .= '<button type="button" class="dropdown-item allocate_fuel" data-target-id=' . $fuel->id . '><div class="row no-gutters align-items-center"><div class="col-2"><i class="ft-plus-circle"></i></div><div class="col-9 offset-1">Allocate Fuel</div></button>';

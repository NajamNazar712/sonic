...
class RetailCashCollectionController extends Controller
{
    ...
    public function retail_list(Request $request)
    {
        ...
        $deliveries = RetailCashDeposit::leftjoin('retail_users as ru', 'ru.id', '=', 'retail_cash_deposits.retail_user_id')
                        ...
                        ->select(['rpn.id', 'retail_cash_deposits.id as retail_pickup_note_id', 'oc.id as hub_id', 'oc.name as hub','a.name as assignee',  'rpn.assigned_at', 'retail_cash_deposits.total_cn as shipments_count',  DB::raw('ROUND(retail_cash_deposits.total_cash) AS amount'),'rf.name as franchise','rf.code as franchise_code','rc.name as center','rc.code as center_code','ru.category', 'rpn.status', 'rtc.name as retail_trax_center_name', 'rtc.code as retail_trax_center_code','retail_cash_deposits.status as rcd_status'])
...
//            })
            ...
            ->editColumn('rcd_status', function ($user) {

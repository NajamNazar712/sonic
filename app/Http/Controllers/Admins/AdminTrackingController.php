                            ...
                            foreach ($shipments as $shipment){
                                ...
                                if($last_scanned_location->exists()){
                                    ...
                                    if($last_scanned_location->admin_id == null && $last_scanned_location->user_id == null){
                                        ...
                                        $screen_location = '-';
                                        $screen_location_id = null;
                                        $scanned_by = '-';
                                        $city = '-';
                                        $scanned_by_id = null;
                                        $scanned_by_user_type = '-';
                                    }
                                    else {
                                        $screen_location = $last_scanned_location->screen_location->name;
                                        $screen_location_id = $last_scanned_location->screen_location->id;
                                        ...
                                        if ($last_scanned_location->user_type == 1) {
                                            ...
                                            if($admin){
                                                ...
                                                $scanned_by_id = $admin->id;
                                                $scanned_by_user_type = 1;
                                            ...
                                            if($sub_user){
                                                ...
                                                $scanned_by = $scanned_by . ' (' . $account_type . ')';
                                                $scanned_by_id = $sub_user->id;
                                                $scanned_by_user_type = 3;
                                            }
                                            else {
                                                $scanned_by_id = null;
                                                $scanned_by_user_type = '-';
                                            ...
                                            if($retail_admin){
                                                ...
                                                $scanned_by = $scanned_by . ' (' . $account_type . ')';
                                                $scanned_by_id = $retail_admin->id;
                                                $scanned_by_user_type = 4;
                                            }
                                            else {
                                                $scanned_by_id = null;
                                                $scanned_by_user_type = '-';
                                ...
                                $shipment_position->scanned_by_id = $scanned_by_id;
                                $shipment_position->scanned_by_user_type = $scanned_by_user_type;
        ...
        ->leftJoin('shipments_journey as sjl', function ($join) use ($sj_from_id, $arrived_sj_to_id,$connection) {
            $join->on('sjl.shipment_id', '=', 's.id')
                ->where(
                    'sjl.id',
                    '=',
                    DB::connection($connection)->raw("(select max(id) from shipments_journey where shipments_journey.shipment_id = s.id and shipments_journey.shipper_status_id IN (2,3,4,5,11,23,53) and shipments_journey.id >= $sj_from_id and shipments_journey.id <= $arrived_sj_to_id)")
                );
        })
        ...
        ->leftJoin('shipments_journey as journey', function ($join) use ($sj_from_id, $arrived_sj_to_id,$connection) {
            $join->on('journey.shipment_id', '=', 's.id')
                ->where(
                    'journey.id',
                    '=',
                    DB::connection($connection)->raw("(select max(id) from shipments_journey where shipments_journey.shipment_id = s.id and shipments_journey.id >= $sj_from_id and shipments_journey.id <= $arrived_sj_to_id )")
                );
        })
        ->leftJoin('scanned_user_types as sp', 'shipment_positions.scanned_by_user_type', '=', 'sp.id')
        ->leftJoin('shipment_scanning_journeys as ssj', function ($join)  use ($sj_from_id, $arrived_sj_to_id,$from,$to){
            $join->on('ssj.shipment_id', '=', 'journey.shipment_id')
                 ->whereRaw("ssj.id = (
                                ...
                                and shipment_scanning_journeys.created_at >= $from and shipment_scanning_journeys.created_at <= $to
                            )");
        })

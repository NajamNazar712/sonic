...
class TeamLeadDashboardController extends Controller
{
    ...
    public function shipment_assign_index()
    {
        ...
        $number_of_closed_ticket_percentage = number_format((count($number_of_closed_tickets) / ($number_of_rv_ticket) * 100));
        $number_of_connected_calls = RvShipmentAssignAgent::whereIn('rv_assign_agent_status_id', [1, 2, 3, 5])->get();
        $number_of_connected_calls_percentage = number_format((count($number_of_connected_calls) / ($number_of_rv_ticket) * 100));
        $number_of_unresponsive_call = RvShipmentAssignAgent::where('rv_assign_agent_status_id', 6)->get();
        $number_of_unresponsive_percentage = number_format((count($number_of_unresponsive_call) / ($number_of_rv_ticket) * 100));
        $number_of_reattempt_call = RvShipmentAssignAgent::where('rv_assign_agent_status_id', 2)->get();
        $number_of_reattempt_percentage = number_format((count($number_of_reattempt_call) / ($number_of_rv_ticket) * 100));
        $number_of_return_confirm_call = RvShipmentAssignAgent::where('rv_assign_agent_status_id', 1)->get();
        $number_of_return_confirm_percentage = number_format((count($number_of_return_confirm_call) / ($number_of_rv_ticket) * 100));
        $number_of_intercept_call = RvShipmentAssignAgent::where('rv_assign_agent_status_id', 3)->get();
        $number_of_intercept_percentage = number_format((count($number_of_intercept_call) / ($number_of_rv_ticket) * 100));
        $number_of_self_collection_call = RvShipmentAssignAgent::where('rv_assign_agent_status_id', 5)->get();
        $number_of_self_collection_percentage = number_format((count($number_of_self_collection_call) / ($number_of_rv_ticket) * 100));

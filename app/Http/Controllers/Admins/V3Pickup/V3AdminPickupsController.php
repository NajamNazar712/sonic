...
class V3AdminPickupsController extends Controller
{
    ...
    public function pickup_request_add(Request $request)
    {
        ...
        if ($pickup_type_id == 2) {
            ...
            //walking process
            $pickup_address_id = ShipperShipmentBookController::add_pickup_address(
                $shipper_id,
                $walkin_address,
                $walkin_name,
                null,
                substr_replace($walkin_contact, '-', 4, 0),
                'info@trax.pk',
                $city_id,
                0,
                true
...
//        dd($statuses);
        ...
        return view('admin.v3_pickups.pending')->with(['riders' => $riders, 'pickup_reasons' => $pickup_reasons, 'shippers' => $shippers, 'pickup_shipment_types' => $pickup_shipment_types, 'time_ranges' => $time_ranges, 'products' => $products, 'services' => $services, 'additional_services' => $additional_services, 'statuses' => $statuses, 'cities' => $cities]);
    }
    ...
    public function pending_requests_list(Request $request)
    {
        ...
        $pickup_requests = V3PickupRequest::join('users as u', 'v3_pickup_requests.shipper_id', '=', 'u.id')
            ...
            ->leftjoin('users as us', function ($query) {
                $query->on('us.id', '=', 'v3_pickup_requests.generated_by')->where('generated_type', 0);
            })
            ->leftjoin('admins as ad', function ($query) {
                $query->on('ad.id', '=', 'v3_pickup_requests.generated_by')->where('generated_type', 1);
            })
            ...
            })->leftJoin('admins as ag', 'ag.id', '=', 'vpa.assigned_by')
    ...
    public function get_shipment_picked_detail(Request $request)
    {
        ...
        // $shipment_picked_detail=Shipment::join('v3_pickup_requests AS pr',function($query){
        //    $query->on(DB::raw('shipments.user_id'),'=','pr.shipper_id')
        //    ->where(DB::raw('shipments.pickup_address_id'),'=','pr.pickup_address_id') ;
        // })->join('shipments_journey as sj',function($query){
        //     $query->on('sj.shipment_id','=','shipments.id');
        // })->where('sj.shipper_status_id',53)->where('pr.pickup_date','=',DB::raw('DATE(sj.created_at)'))
...
//        return view('admin.v3_pickups.schedule_pickup')->with(['riders' => $riders, 'shippers' => $shippers, 'pickup_shipment_types' => $pickup_shipment_types, 'time_ranges' => $time_ranges, 'products' => $products, 'services' => $services, 'additional_services' => $additional_services]);
        return view('admin.v3_pickups.schedule_pickup');
    ...
    public function schedule_approved(Request $request)
    {
        ...
        $regular_pickup->approved_at = $approved_at;
    ...
    public function schedule_rejected(Request $request)
    {
        ...
        $regular_pickup->approved_by = $user_id;
        $regular_pickup->approved_at = $reject_at;
        if ($regular_pickup->save()) {
...
//        $additional_services = V3PickupService::all();
        return view('admin.v3_pickups.pickup_history')->with(['pickup_reasons'=> $pickup_reasons ,'statuses' => $statuses]);

//        return view('admin.v3_pickups.pickup_history')->with(['riders' => $riders, 'pickup_reasons' => $pickup_reasons, 'shippers' => $shippers, 'pickup_shipment_types' => $pickup_shipment_types, 'time_ranges' => $time_ranges, 'products' => $products, 'services' => $services, 'additional_services' => $additional_services, 'statuses' => $statuses]);
    }
    ...
    public function rider_assign(Request $request)
    {
        ...
        foreach ($pickup_request_ids as $pickup_request_id) {
            ...
            if ($pickup_request->status_id != 6) {
                ...
                if (!$existing_pickup_request_attempt->exists()) {
                    ...
                    V3PickupRequestJourneysController::add_pickup_request_journey($pickup_request->id, 2, 1, $admin_id);
        ...
        if (count($allowed_pickup_requests) > 0) {
            ...
            if (count($notification_data) > 0) {
                ...
                return redirect()->back()->with('error', 'Pickup Request(s) already picked!');
            }
        }
    ...
    public function arrival_individual_index(Request $request)
    {
        ...
        return view('admin.v3_pickups.arrival.arrival_individual_weight')->with(['riders' => $riders, 'global_rider_id' => $global_rider_id]);
    }

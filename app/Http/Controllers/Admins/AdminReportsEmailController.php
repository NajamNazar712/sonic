...
class AdminReportsEmailController extends Controller
{
    ...
    static public function month_average($date){
        ...
        $months_average = City::leftjoin('user_shipping_infos as usi', 'usi.city_id', '=', 'cities.id')
            ->leftjoin('shipments as s', function($join) use($new_date_from, $new_date_to) {
                $join->on('s.pickup_address_id', '=', 'usi.id')
                    ->where('s.shipper_status_id', 2)
                    ->whereBetween('s.created_at', [$new_date_from, $new_date_to]);
                    })

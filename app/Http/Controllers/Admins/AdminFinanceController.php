...
';
    ...
    public function invoices_list(Request $request)
    {
        ...
        $invoice = Invoice::leftjoin('users as u', 'invoices.user_id', '=', 'u.id')
            ...
            ->leftjoin('star_shippers as sts','sts.user_id','=','u.id')
            ->leftJoin(DB::raw('(SELECT invoice_id, deposit_date, SUM(amount) as total_received_amount FROM invoice_upload_slips GROUP BY invoice_id) ius_sub'), 'invoices.id', '=', 'ius_sub.invoice_id')
            ...
            'invoices.tax_amount as tax_amount', 'ius_sub.deposit_date as deposit_date', 

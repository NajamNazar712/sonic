...
class AdminFinanceController extends Controller
{
    ...
    public function outstanding_shipments_adjust_in_payment(Request $request) {
        ...
        if ($delivery_note_shipment->exists()) {
            ...
            if ($pending_payment_shipment->exists()) {
                ...
                $this->adjust_payment($pending_payment_shipment->pending_payment_id, $request->id, 0, 0);
            }
            else {
                ...
                if ($done_payment_shipment->exists()) {
                    ...
                    $this->adjust_payment($done_payment_shipment->done_payment_id, $request->id, 1, 0);
                }
            }

            ShipmentsPaymentJourneyController::add($request->id, 4, Auth::id());

            ShipmentsJourneyController::add($request->id, 20, 20, NULL, NULL, NULL, Auth::id());
    ...
    static private function adjust_payment($payment_id, $shipment_id, $payment_type, $adjustment_type) {
        if ($adjustment_type == 0) {
            if ($payment_type == 0) {
                $payment_shipment = PendingPaymentShipment::where('pending_payment_id', $payment_id)->where('shipment_id', $shipment_id)->first();
            }
            else {
                $payment_shipment = DonePaymentShipment::where('done_payment_id', $payment_id)->where('shipment_id', $shipment_id)->first();
            }
    ...
    public function make_payments_store(Request $request) {
        ...
        foreach ($pending_payment_shipment_ids as $pending_payment_id => $pending_payment_shipment_ids) {
            ...
            if ($total_shipments == $selected_shipments) {
                ...
                $pending_payment_shipments = PendingPaymentShipment::where('pending_payment_id', $pending_payment_shipment->pending_payment_id);

                $pending_payment->total_shipments = $pending_payment_shipments->count();
                $pending_payment->delivered_shipments = $pending_payment_shipments->where('type', 0)->count();
                $pending_payment->returned_shipments = $pending_payment_shipments->where('type', 1)->count();
                $pending_payment->adjusted_shipments = $pending_payment_shipments->where('type', 2)->count();

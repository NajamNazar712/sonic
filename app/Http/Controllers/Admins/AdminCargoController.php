...
use App\http\Models\CargoConsignmentShipmentExcel;
...
class AdminCargoController extends Controller
{
    ...
    public function pending_list(Request $request) {
        $shipments = Shipment::join('booking_types as bt', 'shipments.booking_type_id', '=', 'bt.id')
            ...
            ->select('shipments.shipper_status_id', 'shipments.tracking_number', 'shipments.tracking_number as tracking', 'shipments.order_id', 'bt.booking_type as service_type', 'ss.name as status', 'oc.name as origin', 'dc.name as destination', 'u.name as shipper', 'shipments.amount', 'sm.mode as shipping_mode', 'shipments.created_at as booked_at', 'shipments_journey.created_at as arrival_at', 'shipments.booking_type_id', 'usi.poc','csj.created_at as current_status', 'olddc.name as old_destination', 'olddci.name as old_destination_intercept','crm.id as complaint')->where('shipments.shipper_status_id', '!=', 9);
...
//        dispute start for junction
    ...
    public function quick_receive_store(Request $request) {
        ...
        $cargo_consignment_excel = new CargoConsignmentExcel();
        $cargo_consignment_excel->shipments = count($shipment_ids);
        $cargo_consignment_excel->created_by = Auth::id();
...
//        dispute start for junction
    ...
    public function quick_receive_list_index(){
        return view('admin.cargo.quick_receive_index');
    }
    ...
    public function quick_receive_list_ajax(Request $request){
        $cargo_consignment_excel = CargoConsignmentExcel::leftjoin('cargo_consignment_shipment_excels as ccse', 'ccse.cargo_consignment_excel_id', '=', 'cargo_consignment_excels.id')
            ...
            ->select('cargo_consignment_excels.id', 'cargo_consignment_excels.created_at as created_at', 'a.name as created_by','c.name as city','cargo_consignment_excels.shipments as shipments', 'cargo_consignment_excels.cargoes as cargoes', 'cargo_consignment_excels.excel as excel')
        ...
        $datatables = Datatables::of($cargo_consignment_excel)
            ...
            if ($request->get('search_date_from') && $request->get('search_date_to')) {
                ...
                $datatables->whereBetween('cargo_consignment_excels.created_at', [$from,$to]);
            }
        return $datatables->make(true);
    }

    public function quick_receive_list_details(Request $request){
        ...
        return ['status' => 0, 'success' => 'Booked Shipments', 'details' => $details];
    }

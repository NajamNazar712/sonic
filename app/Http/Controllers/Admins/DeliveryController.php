...
//                dd($id);
    ...
    public function history_list(Request $request)
    {
        ...
        $deliveryNoteNumbers = explode(',', (string) $request->get('delivery_note_numbers'));
        $cx = 'reports_2'; // single connection only

        /**
         * ===========================
         * 1) Pre-aggregates (subqueries)
         * ===========================
         */

        // A) Total weight per DN (sum of shipments.actual_weight)
        ...
        // B) Fintech COD per DN
        // NOTE: assumes trax_pay_transactions & fintech_payment_details live in reports_2
        ...
        // D) HBL Konnect total amount per DN
        ...
        // E) Segment buckets per DN (ALL shipments)
        ...
        // F) Delivered segment buckets per DN
        // TODO: If your “delivered” condition differs, change the where() below.
        $aggSegmentsDelivered = DB::connection($cx)
            ->table('delivery_note_shipments as dns')
            ->join('shipments as s', 's.id', '=', 'dns.shipment_id')
            ->join('users as u', 'u.id', '=', 's.user_id')
            ->where('dns.delivered', '=', 1) // <-- adjust to your actual delivered logic if needed
            ->selectRaw("
            dns.delivery_note_id,
        ...
        /**
         * ===========================
         * 2) Main builder (no GROUP BY here)
         * ===========================
         */
        $deliveries = DB::connection($cx)->table('delivery_notes')
            ...
            // Attach pre-aggregates
            ->leftJoinSub($aggWeights, 'aw', function($j){
                $j->on('aw.delivery_note_id','=','delivery_notes.id');
            })
            ->leftJoinSub($aggFintech, 'af', function($j){
                $j->on('af.delivery_note_id','=','delivery_notes.id');
            })
            ->leftJoinSub($aggOneLink, 'aol', function($j){
                $j->on('aol.delivery_note_id','=','delivery_notes.id');
            })
            ->leftJoinSub($aggHBL, 'ahbl', function($j){
                $j->on('ahbl.delivery_note_id','=','delivery_notes.id');
            })
            ->leftJoinSub($aggSegments, 'aseg', function($j){
                $j->on('aseg.delivery_note_id','=','delivery_notes.id');
            })
            ->leftJoinSub($aggSegmentsDelivered, 'adseg', function($j){
                $j->on('adseg.delivery_note_id','=','delivery_notes.id');
            })
            ...
            ->select([
                ...
                DB::raw('COALESCE(aol.one_link_amount_sum, 0) AS one_link_amount'),
            ...
            ->when($request->get('delivery_note_numbers'), function($q) use ($deliveryNoteNumbers){
        ...
        $datatable = Datatables::of($deliveries)
            ...
            // FINTECH charges (uses pre-aggregated fintech_cod_sum)
            ...
            // HBL Konnect % of DNCC — use hbl_amount
            ...
            ->editColumn('shipments_count_link', function ($r) {
                return ($r->shipments_count ?? 0) != 0
                    ? '<button class="btn btn-sm btn-outline-info align-middle">' . $r->shipments_count . '</button>'
                    : 0;
            })
            ->editColumn('delivered_shipments_link', function ($r) {
                return ($r->delivered_shipments ?? 0) != 0
            ...
            ->editColumn('rider', function ($r) {
                return $r->special_rider ? ($r->rider . ' (' . $r->special_rider_name . ')') : $r->rider;
            })
            ...
            ->filterColumn('route', function ($query, $keyword) {
                ...
                if ($keyword !== '') {
                    $query->where(function($q) use ($keyword){
                        $q->whereRaw('LOWER(routes.code) LIKE ?', ["%{$keyword}%"])
                            ->orWhereRaw('LOWER(routes.start) LIKE ?', ["%{$keyword}%"])
                            ->orWhereRaw('LOWER(routes.end) LIKE ?', ["%{$keyword}%"]);
                    });
                } else {
                    $query->whereRaw('false');
                }
            ...
            ->filterColumn('rdns.status', function ($query, $keyword) {
                if ($keyword != 0) {
                    ...
                    $query->whereNull('rdns.status')->orWhere('rdns.status', 0);
                }
            })

            ->addColumn('one_link_payment_count_button', function ($r) {
                return $r->one_link_payment_count !== null
                    ? '<button class="btn btn-sm btn-outline-info align-middle">' . $r->one_link_payment_count . '</button>'
                    : '-';
            })

            // Show summed HBL Konnect amount as a button (replaces per-row SUM)
            ->addColumn('transactions_amount_link', function ($r) {
                return ($r->hbl_amount ?? 0) > 0
                    ? '<button class="btn btn-sm btn-outline-info align-middle">' . $r->hbl_amount . '</button>'
                    : '-';
            })

            // Cash amount = DNCC - HBL Konnect total (as per your prior logic)
            ...
            ->filterColumn('riders.operation_rider_id', function ($query, $keyword) {
                if ($keyword === 'Field In Operations' || strtolower((string) $keyword) === 'field') {
                    return $query->where('riders.operation_rider_id', 1);
                }
                return $query->where('riders.operation_rider_id', 2);
            })

            // These two remain '-', unless you add a pre-joined source that provides shipment_scanning_journey_id
            ->editColumn('location_status', fn($r) => isset($r->location_status) ? (($r->location_status == 1) ? 'On-site' : 'Off-site') : '-')
            ->editColumn('latitude',        fn($r) => '-') // avoid per-row DB hit
            ->editColumn('longitude',       fn($r) => '-') // avoid per-row DB hit

            // Segment exports (read from aggregates)

...
class DeliveryController extends Controller
{
    ...
    public function receive_delivery_status_submit_all(Request $request){
        ...
        if($delivery_note_id != ''){
            ...
            $dncc_amount = Shipment::whereIn('id', $delivered_shipment_ids)->whereIn('shipper_status_id', $delivered_status)->where('charges_mode_id', '!=', 1)->sum('received_amount');
...
//                        }
    ...
    public function receive_delivery_status_delivered(Request $request)
    {
        if (!empty($request->shipment_ids)) {
            ...
            $dncc_amount = Shipment::whereIn('id', $shipment_ids)->whereIn('shipper_status_id', $delivered_status)->where('charges_mode_id', '!=', 1)->sum('received_amount');
...
//                $where = array(7,8,9,10,11,12,14,15,16,18,20,30,35,36,37);
    ...
    public function receive_delivery_verify_status_submit(Request $request)
    {
        ...
        $return_status_array = array(21, 22, 23, 24, 25, 44);
        if ($delivery_note_id != '') {
            foreach ($shipments as $shipment) {

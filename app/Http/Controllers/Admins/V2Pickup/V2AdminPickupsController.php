...
//                                $rider_id = $this->generate_trax_pickup($pickup_request_id);
    ...
    public function rider_receiving_list(Request $request)
    {
        ...
        $rider = V2PickupNote::join('riders as r', 'r.id', '=', 'v2_pickup_notes.rider_id')
            ...
            ->select('v2_pickup_notes.id as note_id', 'v2_pickup_notes.id as id', 'v2_pickup_notes.created_at as date', 'r.name as rider', DB::raw('v2_pickup_notes.bookings AS total_shipment_count'), DB::raw('count(arrsh.id) as total_arrived_count'), DB::raw('count(total_s.id) as rider_picked'))

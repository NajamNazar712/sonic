...
//                   $details['shipper']['email'] = $shipper->email;
    ...
    public function shipment_book_daraz_old(Request $request)
    {
        /********************************NOTE********************************/
    ...
    public function shipment_book_daraz(Request $request)
    {
        ...
        $setting = GlobalSettings::where('type', 'daraz_allowed_user_ids')->first();
        $daraz_account_ids = ($setting && $setting->text) ? array_map('intval', explode(',', $setting->text)) : [7306, 7308, 10389];
        if (!in_array($user_id, $daraz_account_ids)) {
            return response()->json(['status' => 1, 'message' => 'Shipper is not allowed.']);
        }
        ...
        if ($validate->fails()) {
            ...
            $shipment_pre_books = ShipmentPrebook::where('user_id', $user_id)->get();
            $prefix_matched = false;
            foreach ($shipment_pre_books as $spb) {
                $length = strlen($spb->prefix);
                $parts = str_split($request->tracking_number, $length);
                if ($spb->prefix == $parts[0]) {
                    $prefix_matched = true;
                    break;
                }
            }
            ...
            //Return Address
            $return_city_id = $request->return_city_id;
            $normalized_return_phone = $this->phone_number($request->return_phone_number);
            $return_address = UserShippingInfo::where('user_id', $user_id)->where('vendor', $request->return_vendor)->where('city_id', $return_city_id)->where('phone', $normalized_return_phone);
            if ($return_address->exists()) {
                $return_address = $return_address->first();
                $return_address_id = $return_address->id;
            } else {
            ...
            if ($amount == 0 && Carbon::parse($user_type->activated_at)->lt(Carbon::now()->subDays(1)) && !PendingPayment::check_negative_payable($user_id, $user_type['account_type_id'])) {
                $allowed = NegativePayableAllowShipperZeroCod::isAllowed($user_id);
                if (!$allowed) {
                    return response()->json([
                        'status' => 1,
            ...
            foreach ($nsa_keywords as $nsa) {
                foreach ($str_arr as $arr_value) {
                    if (strtolower($nsa) == strtolower($arr_value)) {
                        $msg_string = $msg_string ? $msg_string . ', ' . $arr_value : $arr_value;
                    }
                }
            }
            ...
            if ($msg_string !== null) {
                NotificationsController::send(32, $shipment_id, $msg_string);
                $return_array['non_service_area'] = 'A Possible Address Anomaly: ' . $msg_string . ' Detected! In case of, Out Of Service Area: Additional charges may apply and Non Service Area: Shipment may be returned. For assistance, Call: 021-38772222.';
            }

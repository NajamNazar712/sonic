    ...
    public function handle($request, Closure $next)
    {
        ...
        if ($api_token) {
            ...
            if ($admin->exists()) {
                ...
                if ($admin->status) {
                    if($admin->employee){
                        $request->request->add(['rider_id' => $admin->id, 'trax_id' => $admin->trax_id, 'admin_employee' => $admin->employee->id]);
                    }
                    else{
                        $request->request->add(['rider_id' => $admin->id, 'trax_id' => $admin->trax_id]);
                    }

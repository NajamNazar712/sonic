...
class ProcessShipmentBookingDB implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach($this->booking as $booking) {
            ...
            if(array_key_exists("open_shipment",$booking)){
                if (strtolower($booking['open_shipment']) == 'yes') {
                    $open_shipment = 1;
                } else {
                    $open_shipment = 0;
                }
            ...
            if($service_type_id == 5){
                if (!empty(trim($booking['consignee_email_address']))) {
                    $user_email_id = $booking['consignee_email_address'];
                }
                else{
                    $user = User::find($user_id);
                    $user_email_id = $user->email;
                }
                $pickup_city_id = City::where('name', $booking['consignee_city_name'])->first()->id;

                $phone_number = ShipperShipmentBookController::phone_number($booking['consignee_phone_number_1']);
                $pickup_address_id = ShipperShipmentBookController::add_pickup_address($user_id, $booking['consignee_address'], $booking['consignee_name'], NULL, substr_replace($phone_number, '-', 4, 0), $user_email_id, $pickup_city_id, 0, TRUE);

                $pickup_delivery_address_id = $booking['pickup_address_id'];
                ...
                $pickup_city_id = UserShippingInfo::find($booking['pickup_address_id'])->city_id;
                ...
                $consignee_city_id = City::where('name', $booking['consignee_city_name'])->first()->id;
                $consignee_name = $booking['consignee_name'];
                $consignee_address = $booking['consignee_address'];
                $consignee_phone_number_1 = ShipperShipmentBookController::phone_number($booking['consignee_phone_number_1']);

                if (!empty(trim($booking['consignee_phone_number_2']))) {
                    $consignee_phone_number_2 = ShipperShipmentBookController::phone_number($booking['consignee_phone_number_2']);
                }
                else{
                    $consignee_phone_number_2 = null;
                }
                ...
                if (!empty(trim($booking['consignee_email_address']))) {
                    $consignee_email_address = $booking['consignee_email_address'];
                } else {
                    $consignee_email_address = NULL;
                }
                $payment_mode_id = $booking['payment_mode_id'];
                if (array_key_exists("self_collection",$booking))
                {
                    if (strtolower($booking['self_collection']) == 'yes') {
            ...
            $charges_mode_id = $booking['charges_mode_id'];
            if (!empty(trim($booking['order_id']))) {
                $order_id = $booking['order_id'];
            } else {
                $order_id = NULL;
            }

            if (!empty(trim($booking['special_instructions']))) {
            ...
            $shipping_mode_id = $booking['shipping_mode_id'];

            if ($shipping_mode_id == 4) {
                $same_day_timing_id = $booking['same_day_timing_id'];
            } else {
            ...
            elseif ($service_type_id == 5){
                ...
                $parcel_value = $booking['parcel_value'];
            }
            $pieces_quantity = 1;

            if($service_type_id == 1){
                if (isset($booking['pieces_quantity'])) {
                    $pieces_quantity = $booking['pieces_quantity'];
                }
            }

            $return_address_id = NULL;
            if($service_type_id == 1 || $service_type_id == 2){
                if (isset($booking['return_address_id'])) {
                    $return_address_id = $booking['return_address_id'];
                }
            }
            $business_category_id = $booking['business_category_id'];
            ...
            if ($booking['account_type_id'] == 1) {
                ...
                if ($delivery_type_id == 2) {
                    $consignee_address = 'TRAX Office ' . $booking['consignee_city_name'];
                }
                ...
                if($booking['substitute_user_id'] != null){
                    $substitute_user_shipment = new SubstituteUserShipment();
                    $substitute_user_shipment->substitute_user_id = $booking['substitute_user_id'];
                    $substitute_user_shipment->shipment_id = $shipment_id;
                    $substitute_user_shipment->save();
                }

            if($booking['prefix'] != NULL){
                $tracking_number = ShipperShipmentBookController::generate_prefix_tracking_number($shipment_id, $booking['order_id']);
            }
            else{
                $tracking_number = ShipperShipmentBookController::generate_tracking_number($shipment_id, $pickup_city_id, $consignee_city_id);
            }

            if($booking['business_category_id'] == 2){
                $international_shipment = new InternationalShipment();
                $international_shipment->shipment_id = $shipment_id;
                $international_shipment->postal_code = $booking['postal_code'];
                $international_shipment->save();
            }

            if($booking['order_date'] != null){
                $order_date = new ShipmentOrderDate();
                $order_date->shipment_id = $shipment_id;
                $order_date->order_date = $booking['order_date'];
                $order_date->save();
            }

            if($booking['shipper_reference_number_1'] != null || $booking['shipper_reference_number_2'] != null || $booking['shipper_reference_number_3'] != null || $booking['shipper_reference_number_4'] != null || $booking['shipper_reference_number_5'] != null){
                $shipper_reference = new ShipmentShipperReference();
                $shipper_reference->shipment_id = $shipment_id;
                if($booking['shipper_reference_number_1'] != null) {
                    $shipper_reference->reference_1 = $booking['shipper_reference_number_1'];
                }
                if($booking['shipper_reference_number_2'] != null) {
                    $shipper_reference->reference_2 = $booking['shipper_reference_number_2'];
                }
                if($booking['shipper_reference_number_3'] != null) {
                    $shipper_reference->reference_3 = $booking['shipper_reference_number_3'];
                }
                if($booking['shipper_reference_number_4'] != null) {
                    $shipper_reference->reference_4 = $booking['shipper_reference_number_4'];
                }
                if($booking['shipper_reference_number_5'] != null) {
                    $shipper_reference->reference_5 = $booking['shipper_reference_number_5'];
                }
                $shipper_reference->save();
            }

            if ($service_type_id == 1 || $service_type_id == 5) {
                $item_product_type_id = $booking['item_product_type_id'];

                if (!empty(trim($booking['item_description']))) {
                ...
                if (strtolower($booking['item_insurance']) == 'yes') {
                    $item_price = str_replace(',', '', $booking['item_price']);
                    $item_insurance = TRUE;
                } else {
                    $item_price = NULL;
                ...
                $replacement_item_product_type_id = $booking['replacement_item_product_type_id'];

                if (!empty(trim($booking['replacement_item_description']))) {
                    $replacement_item_description = $booking['replacement_item_description'];
                } else {
                    $replacement_item_description = NULL;
                }

                $replacement_item_quantity = $booking['replacement_item_quantity'];
                ...
                $item_product_type_id = $booking['item_product_type_id_1'];

                if (!empty(trim($booking['item_description_1']))) {
                    $item_description = $booking['item_description_1'];
                } else {
                    $item_description = NULL;
                }

                $item_quantity = $booking['item_quantity_1'];

                if (strtolower($booking['item_insurance_1']) == 'yes') {
                    $item_insurance = TRUE;
                } else {
                    $item_insurance = FALSE;
                }
                $item_price = str_replace(',', '', $booking['item_price_1']);

                $item_type = 2;
                $try_and_buy_cod_amount = $try_and_buy_cod_amount + intval($item_price);
                ShipperShipmentBookController::add_item($shipment_id, $item_product_type_id, $item_description, $item_quantity, $item_price, $item_insurance, $item_type);

                if($booking['item_product_type_id_2'] != null){
                    $item_product_type_id = $booking['item_product_type_id_2'];

                    if (!empty(trim($booking['item_description_2']))) {
                        $item_description = $booking['item_description_2'];
                    } else {
                        $item_description = NULL;
                    }

                    $item_quantity = $booking['item_quantity_2'];


                    if (strtolower($booking['item_insurance_2']) == 'yes') {
                        $item_insurance = TRUE;
                    } else {
                        $item_insurance = FALSE;
                    }
                    $item_price = str_replace(',', '', $booking['item_price_2']);
                ...
                if($booking['item_product_type_id_3'] != null){
                    $item_product_type_id = $booking['item_product_type_id_3'];

                    if (!empty(trim($booking['item_description_3']))) {
                        $item_description = $booking['item_description_3'];
                    } else {
                        $item_description = NULL;
                    }

                    $item_quantity = $booking['item_quantity_3'];

                    if (strtolower($booking['item_insurance_3']) == 'yes') {
                        $item_insurance = TRUE;
                    } else {
                        $item_insurance = FALSE;
                    }
                    $item_price = str_replace(',', '', $booking['item_price_3']);
                ...
                if($booking['item_product_type_id_4'] != null){
                    $item_product_type_id = $booking['item_product_type_id_4'];

                    if (!empty(trim($booking['item_description_4']))) {
                        $item_description = $booking['item_description_4'];
                    } else {
                        $item_description = NULL;
                    }

                    $item_quantity = $booking['item_quantity_4'];

                    if (strtolower($booking['item_insurance_4']) == 'yes') {
                        $item_insurance = TRUE;
                    } else {
                        $item_insurance = FALSE;
                    }
                    $item_price = str_replace(',', '', $booking['item_price_4']);
                ...
                if($booking['item_product_type_id_5'] != null){
                    $item_product_type_id = $booking['item_product_type_id_5'];

                    if (!empty(trim($booking['item_description_5']))) {
                        $item_description = $booking['item_description_5'];
                    } else {
                        $item_description = NULL;
                    }

                    $item_quantity = $booking['item_quantity_5'];

                    if (strtolower($booking['item_insurance_5']) == 'yes') {
                        $item_insurance = TRUE;
                    } else {
                        $item_insurance = FALSE;
                    }
                    $item_price = str_replace(',', '', $booking['item_price_5']);
            ...
            if(!empty($booking['nsas']) && $booking['nsa']) {
                $present = '';
                $address_array = preg_split("/[ ,]+/", $booking['consignee_address']);

                foreach ($booking['nsas'] as $nsa) {

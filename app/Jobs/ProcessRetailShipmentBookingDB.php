...
class ProcessRetailShipmentBookingDB implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $consignee_phone_number_1 = $this->booking['consignee_phone_no'];
        ...
        $shipping_mode_check = $this->booking['shipping_mode'];
        if ($shipping_mode_check == 1) {
//            if($this->booking['business_category']== 1)
//            {
                $consignee_city_id = $this->booking['domestic_overland_destination'];
//            }
//            else{
...
//            if($this->booking['business_category'] == 1) {
                $consignee_city_id = $this->booking['domestic_destination'];
//            }
//            else{
//                $consignee_city_id = $this->booking['international_destination'];
//            }
...
//            }
        ...
        $business_category_id = $this->booking['business_category'];

        $charges_mode_id = 1;

        if (strtolower($this->booking['volumetric_weight']) == "on") {
        ...
        $product_type_id = $this->booking['product'];
...
//        }
        ...
        if($this->booking['shipping_mode'] == 1){
//            if($this->booking['business_category'] == 1) {
                $destination = $this->booking['domestic_overland_destination'];
//            }
//            else{
...
//            if($this->booking['business_category'] == 1) {
                $destination = $this->booking['domestic_destination'];
//            }
//            else{
//                $destination = $this->booking['international_destination'];
//            }
        ...
        $shipper_info = RetailShipperInfo::where('shipper_phone_no', $this->booking['shipper_phone_no']);
        if($shipper_info->exists()){
            $shipper_info = $shipper_info->first();
            $shipper_info->shipper_phone_no = $this->booking['shipper_phone_no'];
            ...
            if ($this->booking['iban_no'] != null && $this->booking['account_no'] != null && $this->booking['bank'] != null) {
                $shipper_info->bank_id = $this->booking['bank'];
                $shipper_info->iban = $this->booking['iban_no'];
        ...
        $retail_shipment->product_type_id = $this->booking['product'];
        $retail_shipment->shipping_mode = $this->booking['shipping_mode'];
        $retail_shipment->destination = $destination;
        $retail_shipment->payment_mode_id = $this->booking['payment_mode'];
        $retail_shipment->shipper_phone_no = $this->booking['shipper_phone_no'];
        ...
        $retail_shipment->trax_box_id = $this->booking['trax_box'];
...
//        $retail_shipment->cash_handling_charges = $this->booking[cash_handling_charges;
        ...
        $cash_deposit_shipment->shipping_mode_id = $this->booking['shipping_mode'];

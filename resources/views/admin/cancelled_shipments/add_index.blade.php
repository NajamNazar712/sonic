    ...
    <div class="card">
        <div class="card-content" aria-expanded="true">
            <div class="card-body">
                ...
                <form id="cancelled_shipment_form" class="form-inline mb-1 justify-content-center" novalidate="novalidate">
                ...
                <form id="update_cancelled_form" action="{{route('admin.cancelled_shipments.add.store')}}" class="form-horizontal" method="POST">
                    {{ csrf_field() }}
                    ...
                    <div class="row justify-content-center">
                        <div class="col-3">
                            <button type="submit" class="btn btn-primary btn-block" disabled id="update_cancelled_form_submit">Submit</button>
...
@section('js')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#cancelled_shipment_form input.tracking_number').inputmask({
                'alias': 'integer',
                'allowMinus': false,
                'allowPlus': false
            });
            {{--var cities_array = @json($cities);--}}
            $('#cancelled_shipment_form').validate({
                errorClass: 'danger',
                ...
                submitHandler: function(form) {
                    $('#cancelled_shipment_form button.add').prop('disabled', true);
                    ...
                    if (table.columns('.tracking_number').data().eq(0).indexOf(parseInt(tracking_number)) === -1) {
                        ...
                        $.ajax({
                            url: '{!! route('admin.cancelled_shipments.add.shipment_info') !!}',
                            method: 'POST',
                            data: {
                            ...
                            .done(function(data) {
                                if (data.status == 1) {
                                    ...
                                    if (index === -1) {
                                        ...
                                        $('#cancelled_shipment_form button.add').prop('disabled', false);

                                        $('#update_cancelled_form_submit').prop('disabled', false);
            ...
            $('#update_cancelled_form').validate({
                errorClass: 'danger',
                successClass: 'success',
                errorPlacement: function(error, element) {
                ...
                submitHandler: function(form) {
                    ...
                    swal({
                        title: 'Are You Sure?',
                        text: 'Select Yes to Cancel Shipments!',
                        icon: 'warning',
                        buttons: {
                            cancel: {
                                text: 'No',
                        ...
                        if(confirm){
                            blockPagePermanently();
                            $('#update_cancelled_form button[type="submit"]').attr('disabled', 'disabled');
                            $('#update_cancelled_form input#shipment_ids').val(shipment_ids);
                            form.submit();
                        }
                    });
                }

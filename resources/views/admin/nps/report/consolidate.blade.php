...
@section('title', 'NPS Consolidate Report')

@section('content')
    <h1>NPS Consolidate Report</h1>
                                    ...
                                    <div class="col-4">
                                        <div class="form-group input-group">
                                            ...
                                            <input type="text" name="requested_from_date"
                                                   ...
                                                   id="requested_from_date" placeholder="Survey Start Date">
                                        </div>
                                    </div>
                                    <div class="col-4">
                                        <div class="form-group input-group">
                                            ...
                                            <input type="text" name="requested_to_date"
                                                   class="form-control bg-primary border-primary white rounded-right"
                                                   id="requested_to_date" placeholder="Survey End Date">
                                        </div>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#datatable').DataTable({
                ...
                deferLoading: 0,
                ajax: {
                    url: '{{ route('admin.nps.consolidate_report_list') }}',
                    type: "get",
                    data: function (d) {
                        d['_token'] = "{{csrf_token()}}";
                ...
                columns: [
                    ...
                    {data: 'total_shippers', name: 'total_shippers', class: 'align-middle total_shippers' ,sortable:false,orderable: false},
                    {data: 'total_response', name: 'total_response', class: 'align-middle total_response' ,sortable:false,orderable: false},
                    {data: 'response_percentage', name: 'response_percentage', class: 'align-middle response_percentage' ,sortable:false,orderable: false,
                        render: function (data, type, row) {
                           var total_shippers = row.total_shippers;
                           var total_response = row.total_response;
                           if(total_shippers && total_response){
                               return ((total_response/total_shippers)*100).toFixed(2);
                           }else{
                               return  '0'
                           }
                    ...
                    {data: 'promoters', name: 'promoters', class: 'align-middle promoters greenn',sortable:false,orderable: false},
                    {data: 'passive', name: 'passive', class: 'align-middle passive yelloww',sortable:false,orderable: false},
                    {data: 'detractor', name: 'detractor', class: 'align-middle detractor redd',sortable:false,orderable: false},
                ...
                initComplete: function () {
                    ...
                    this.api().columns().every(function (column_id) {
                        ...
                        if ($(header).is('.action') || $(header).is('.serial_number') || $(header).is('.promoters') || $(header).is('.passive') || $(header).is('.detractor')|| $(header).is('.total_shippers')|| $(header).is('.total_response')|| $(header).is('.response_percentage')) {
                            $(td).appendTo($(search));
                        }

...
@section('title', 'Quick Arrival of Shipments')
				...
				<div class="card">
					<div class="card-content" aria-expanded="true">
						<div class="card-body">
							...
							<form id="arrival_of_shipments_form" class="form-horizontal text-center" method="POST" action="{{ route('admin.pickups.quick_arrival_of_shipments.store') }}" novalidate="novalidate">
								{{ csrf_field() }}
	...
	<script>
		$(document).ready(function() {
			...
			$('#add_shipment_form').validate({
				...
				submitHandler: function(form) {
					...
					if (table.columns('.tracking_number').data().eq(0).indexOf(parseInt(tracking_number)) === -1) {
						$.ajax({
							url: '{!! route('admin.pickups.quick_arrival_of_shipments.shipment_details') !!}',
							method: 'POST',
							data: {
			...
			$('#datatable tbody').on('click', 'tr td.remove button', function() {
				...
				$.ajax({
					url: '{!! route('admin.pickups.quick_arrival_of_shipments.shipment_remove') !!}',
					method: 'POST',
					data: {
						'id': id,
						'_token': '{{ csrf_token() }}'

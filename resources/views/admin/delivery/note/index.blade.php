                    ...
                    if(rowsCount === 0) {
                        $.ajax({
                            ...
                            if(data.status === 1){
                                ...
                                table.row.add([rowNo,data.tracking_number,data.destination,data.consignee_name,data.phone,notification_check,rider_information,data.address,data.amount,data.service_type,data.shipment_status,rider_name,data.remarks,remove]).node().id = data.shId;
                                table.draw(false);

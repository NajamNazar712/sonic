                    ...
                    if(rowsCount === 0) {
                        $.ajax({
                            ...
                            if(data.status === 1){
                                ...
                                table.row.add([rowNo,data.tracking_number,data.destination,data.consignee_name,data.phone,notification_check,rider_information,data.address,data.amount,data.service_type,data.shipment_status,rider_name,data.remarks,remove]).node().id = data.shId                                table.draw(false);
                        ...
                        if(is_indexed === -1){
                            ...
                            $.ajax({
                                ...
                                if(data.status === 1){
                                    ...
                                    var remove = '<a href="javascript:void(0);" class="btn btn-icon btn-danger deliverynoterow"><i class="la la-close"></i></a>';
                                    table.row.add([rowNo+1,data.tracking_number,data.destination,data.consignee_name,data.phone,notification_check,rider_information,data.address,data.amount,data.service_type,data.shipment_status,data.remarks,remove]).node().id = data.shId;                                    table.draw(false);
                                    shipment_ids.push(data.shId);
                                    tracking_ids.push(data.tracking_number);
                                    notification_ids.push(1);
                                    rider_info_ids.push(1);

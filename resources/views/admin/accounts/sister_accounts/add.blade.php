...
@section('title', 'Add Sister Account')
                ...
                <div class="card">
                    <div class="card-content" aria-expanded="true">
                        <div class="card-body">
                            ...
                            <form id="merge_account_form" class="" method="post" action="{{ route('admin.accounts.sister_account.add.submit') }}" novalidate="novalidate">
                                {{ csrf_field() }}
                                <div class="row justify-content-center">
                                    <input type="hidden" name="account_ids" id="account_ids">
                                    <div class="form-group col-3">
                                        <input type="text" name="group_name" class="form-control" placeholder="Group Name*" id="group_name" data-rule-required="true" data-msg-required="Group Name is required">
                                    </div>
                                    <div class="col-3">
...
@section('js')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var first_account = @json($first_account);
            var account_ids = [];
            ...
            var first_account_id = ''+first_account.id+'';
            var first_account_id_pad = first_account_id.padStart(6, '0');

            var status_no = first_account.status;
            var status = '';
            if(status_no == 0){
            ...
            var row = table.row.add([first_count, first_account_id_pad, first_account.company_name, first_account.city, first_account.contact_name, first_account.phone, first_account.address, first_account.email, first_account.product_type, status, first_account.tagged_to, action]).node().id = first_account.id;
            account_ids.push(first_account.id);
            table.draw();

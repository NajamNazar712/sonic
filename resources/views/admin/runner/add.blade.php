...
@section('title', 'Add ' . $runner->name . ' On Route')

@section('content')
    <h1 class="mb-1">
        Add {{$runner->name}} On Route
    </h1>
    ...
    <div class="card">
        <div class="card-content" aria-expanded="true">
            <div class="card-body">
                ...
                <form id="runner_route_form" class="form-horizontal" action="{{ route('admin.runner.add.submit') }}" method="POST" novalidate="novalidate">
                    @csrf
                    <input type="hidden" name="id" value="{{$runner->id}}">
                    <input type="hidden" name="status" id="status" value="">
                    <div class="row justify-content-center">
                        <div class="col form-group">
                            <input type="text" class="form-control" name="driver_name" id="driver_name" placeholder="Driver Name*" value="" data-rule-required="true" data-msg-required="Driver Name is required">
                        </div>
                        <div class="col form-group">
                            <input type="text" class="form-control" name="vehicle_number" id="vehicle_number" placeholder="Vehicle Number*" value="" data-rule-required="true" data-msg-required="Vehicle Number is required">
                        </div>
                        <div class="col form-group">
                            <input type="text" class="form-control phone" name="contact_number" id="contact_number" placeholder="Contact Number*" value="" data-rule-required="true" data-msg-required="Contact Number is required">
                        </div>
                    </div>
...
@section('js')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var first_junction_id = @json($junctions[0]->city_id);
            var last_junction_id = @json($last_junction);
            var first_junction = @json($junctions[0]->city_name);
            var first_origin = '<div class="form-group input-group"><select name="origin[1]" id="origin_1" class="select2 form-control" data-rule-required="true" data-msg-required="Origin is required">' +
                '<option value="'+ first_junction_id +'">'+ first_junction +'</option></select></div>';
            var first_destination = '<div class="form-group input-group"><select name="destination[1]" id="destination_1" class="select2 form-control" data-rule-required="true" data-msg-required="Destination is required"></select></div>';
            var departure_time = '<div class="row"><div class="col form-group input-group"><input type="text" name="departure_date[1]" id="departure_date_1" class="form-control rounded-right pickadate date" value="" placeholder="Departure Date*" data-rule-required="true" data-msg-required="Departure Date is required"></div><div class="col form-group input-group">' +
            ...
            $.each(junctions,function (key,value) {
                ...
                if(index === -1){
                    ...
                    $('#destination_1').append(newOption).trigger('change');
                    $('#destination_1').val('').trigger('change');
                }

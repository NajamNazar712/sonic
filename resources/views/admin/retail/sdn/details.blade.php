...
@section('content')
    <h1 class="mb-1">
        Station Deposit Note ({{str_pad($sdn_id, 6, '0', STR_PAD_LEFT)}})
    </h1>
        ...
        $(document).ready(function () {
            jQuery.fn.DataTable.Api.register( 'buttons.exportData()', function ( options ) {
                if ( this.context.length ) {
                    ...
                    var jsonResult = $.ajax({
                        url: '{{ route('admin.delivery.sdn.retail.ajax',['id'=>$sdn_id]) }}',
                        data: {
                            'page': 'all'
                        },
            ...
            var table = $('#datatable').DataTable({
                ...
                serverSide: true,
                ajax: '{{ route('admin.delivery.sdn.retail.ajax',['id'=>$sdn_id]) }}',
                order: [[1, 'desc']],
                columns: [

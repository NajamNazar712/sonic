...
@section('title','Pending Return Transfer Note Requests')

@section('content')
    <h1 class="mb-1">
        Pending Return Transfer Note Requests
    </h1>
        ...
        $(document).ready(function () {
            jQuery.fn.DataTable.Api.register('buttons.exportData()', function (options) {
                if (this.context.length) {
                    ...
                    var jsonResult = $.ajax({
                        ...
                        url: '{{ route('admin.return_transfer_note.rider_request.list') }}',
                        data: params,
                        async: false
                    });
            ...
            var table = $('#datatable').DataTable({
                ...
                columns: [
                    ...
                    {data: 'request_note_id_padded', name: 'rider_return_transfer_note_requests.id', class: 'align-middle request_note'},
                    ...
                    {data: 'date', name: 'rider_return_transfer_note_requests.created_at', class: 'align-middle date'},
                    {data: 'admin_name', name: 'ad.name', class: 'align-middle admin_name'},
                    {data: 'updated', name: 'rider_return_transfer_note_requests.updated_at', class: 'align-middle updated_at'},
            ...
            $('#datatable tbody').on('click', 'tr td.shipments_count_link button', function () {
                ...
                $.ajax({
                    url: '{!! route('admin.return_transfer_note.rider_request.shipments') !!}',
                    method: 'POST',
                    data: {
                        '_token': '{{ csrf_token() }}',
                        'request_note_id': id
            ...
            {{--            if (confirm) {--}}
            {{--                $.ajax({--}}
            {{--                    url: '{!! route('admin.return_transfer_note.receive.reassign_rider') !!}',--}}
            {{--                    method: 'post',--}}
            {{--                    data: {--}}
            {{--                        '_token': '{{ csrf_token() }}',--}}
            {{--                        'rider': rider,--}}

...
@section('title','Update Return Transfer Request')

@section('content')
    <h1 class="mb-1">
        Update Return Transfer Request
    </h1>
            ...
            var table = $('#datatable').DataTable({
                ...
                buttons: [{
                    ...
                    action: function (e, dt, node, config) {
                        ...
                        //$('input:hidden[name=return_note]').val(selected_rows);
                        ...
                        if(selected_rows.length === 0){
                            ...
                            swal({
                                ...
                                if (confirm) {
                                    $.ajax({
                                        url: '{!! route('admin.return_transfer_note.rider_request.update.remove.bulk') !!}',
                                        method: 'POST',
                                        data: {
                                            '_token': '{{ csrf_token() }}',
                                        ...
                                        if(data.status == 0){
                                            ...
                                            setTimeout(function() {
                                                window.location.href = '{{ route('admin.return_transfer_note.rider_request.index') }}';
                                            }, 2500);
                                        }
                                        else{
                ...
                serverSide: true,
                ajax: '{{ route('admin.return_transfer_note.rider_request.update.list',['id'=>$request_note_id]) }}',
                rowId: 'shId',
                order: [[1, 'desc']],
                columns: [
            ...
            $('body').on('click','a.requestnoterow',function () {
                ...
                swal({
                    ...
                    if (confirm) {
                        ...
                        $.ajax({
                            url:'{{route('admin.return_transfer_note.rider_request.update.remove')}}',
                            type:'POST',
                            data: {
                ...
                submitHandler: function(form) {
                        ...
                        if (tracking) {
                            ...
                            $.ajax({
                                ...
                                url:'{!! route('admin.return_transfer_note.shipment_details') !!}',
                                method:'POST',
                                data:{
                                    'tracking':tracking,
                                    '_token': '{{ csrf_token() }}',
                                ...
                                if(data.status == 0){
                                   var request_note_id = $('#request_note').val();
                                    $.ajax({
                                        ...
                                        url: '{!! route('admin.return_transfer_note.rider_request.add.shipments') !!}',
                                        method: 'POST',
                                        data: {

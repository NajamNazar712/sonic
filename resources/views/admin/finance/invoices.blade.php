			...
			<div class="card-body">
				...
				<div class="modal fade text-left" id="addAdjustmentModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="addAdjustmentModal"
					...
					<div class="modal-dialog modal-md" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h4 class="modal-title" id="">Add Adjustment</h4>
							</div>
							<div class="modal-body">
								<form id="invoice_add_adjustment_form" class="form text-center" action="{{route('admin.finance.invoices.add_adjustment')}}" method="post">
									@csrf
									<input type="hidden" name="invoice_id" id="invoice_id"/>
									<div class="col form-group">
										<label>Amount</label>
										<input type="text" id="adjustment_amount" name="adjustment_amount" class="form-control" placeholder="Enter Amount*" data-rule-required="true" data-msg-required="Amount is required" min="1">
									</div>
									<div class="col form-group">
										<label>Reason</label>
									...
									<div class="col form-group">
										<label>Remarks</label>
										<textarea class="form-control" id="adjustment_remarks" name="adjustment_remarks"></textarea>
									</div>


									<div class="modal-footer">
										<button type="submit" class="btn btn-success" id="adjustmentSubmit">Submit</button>
				...
				<div class="modal fade text-left" id="ViewAdjustment" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="ViewAdjustment"
					 aria-hidden="true">
					<div class="modal-dialog modal-lg" role="document">
						<div class="modal-content">
							<div class="modal-header bg-primary white">
								<h4 class="modal-title white">Adjustment View</h4>
								<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
								</button>
							</div>
							<div class="modal-body text-center">
								<table class="table table-bordered datatable" id="adjustment_table" style="z-index: 3;">
									<thead>
									<tr role="row" class="bg-primary white">
				...
				$(document).ready(function () {
					...
					var table = $('#datatable').DataTable({
						...
						rowCallback: function(row, data, index) {
							...
							if (data.account_type == 2 && (data.is_id == 1 || data.is_id == 2 || data.is_id == 4) && total == parseInt(data.total_invoice_amount)) {
					...
					$('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item', function() {
						...
						else if ($(this).hasClass('add_adjustment')) {

							$('#addAdjustmentModal #invoice_id').val(id);
					...
					$('#uploadDepositSlip').on('shown.bs.modal', function (event) {
						...
						function add_row() {
							...
							var amount = '<input class="form-control form-control-sm money" type="text" id="amount_'+rows_count+'" name="amount['+rows_count+']" data-rule-required="true" data-msg-required="Amount is required" placeholder="Amount*" min="1">';
							...
							deposit_table.row.add([0, date_input,bank_select,amount,deposit_slip,remove]).node().id = rows_count;
					...
					$('#invoice_add_adjustment_form').validate({
						errorClass: 'danger',
						successClass: 'success',
						errorPlacement: function(error, element) {
							error.addClass('w-100').appendTo(element.parent('.form-group'));
					...
					var adjustment_table;
					$('body').on('click','.adjustment_view', function () {
						var id = $(this).parents('tr').attr('id');
						if(id){
							$.ajax({
								url:'{!! route('admin.finance.invoices.adjustment_view') !!}',
								type:'POST',
								data: {
									'invoice_id':id,
									'_token': '{{ csrf_token() }}'
								...
								if(data.status){
									...
									adjustment_table = $('#adjustment_table').DataTable({
										...
										columns: [
											{orderable: false, searchable: false, name: 'serial_number', class: 'align-middle serial_number', targets: 0, render: function (data, type, row) {return '';}},
											{name: 'amount', class: 'align-middle date amount-width form-group'},
											{name: 'reason', class: 'align-middle reason form-group'},
											{name: 'remarks', class: 'align-middle remarks form-group'},
											{name: 'added_by', class: 'align-middle added_by form-group'},
											{name: 'adjusted_date', class: 'align-middle adjusted_date form-group'}
										],

										rowCallback: function(row, data, index) {
											var info = adjustment_table.page.info();
									...
									$.each(data.adjustments, function (index, value) {
										adjustment_table.row.add([0,value.amount, value.reason,value.remarks,value.added_by, value.adjusted_date]);
										adjustment_table.draw(true);
									});
								}
							});
						}
					...
					$('#ViewAdjustment').on('hidden.bs.modal', function () {
						adjustment_table.clear();
						adjustment_table.destroy();
					});

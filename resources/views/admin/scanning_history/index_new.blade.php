...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            var scanningHistoryData = @json($data);
            ...
            function drawHistory(data) {
                ...
                if (data.history !== undefined) {
                    ...
                    $.each(data.history, function(index, history) {
                        ...
                        shipment += '<td>' + history.city + '</td>';
                        shipment += '<td>' + history.area + '</td>';
                        shipment += '<td>' + (history && history.area && history.area_log && history.area_log.location_status != null
                            ? (history.area_log.location_status === 1 ? 'On-site' : 'Off-site')
                            : '-') + '</td>';
                        shipment += '<td>' + history.rider_picked + '</td>';
                        shipment += '<td>' + history.updated_via + '</td>';
            ...
            if (scanningHistoryData) {
                drawHistory(scanningHistoryData);
            }

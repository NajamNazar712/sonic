...
@section('content')
    ...
    {{--Edit Modal--}}
    <div class="modal fade" id="EditChildCNIssueRiderModal" data-backdrop="static" role="dialog" aria-labelledby="EditChildCNIssueRiderModal"
         aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Update Child CN Issue To Riders</h4>
                ...
                <div class="modal-body text-center">
                    ...
                    <form method="post" id="edit_child_cn_issue_rider_form"
                          action="{{ route('admin.logistic.cn.child_issue_to_rider.update') }}"
                          class="form-horizontal mb-1" novalidate="novalidate">
                        @csrf
                        @method('put')
                        <div class="row">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#edit_rider_id_select').prepend('<option value="" selected="selected">Select Rider</option>').select2({
                placeholder: 'Select Rider',
                width: '100%',
                dropdownParent:$('#EditChildCNIssueRiderModal')
            });
            ...
            var table = $('#datatable').DataTable({
                ...
                @if (session('role_id') == 1 || count(array_intersect([970], session('permissions'))) !== 0)
            ...
            $("#edit_child_cn_issue_rider_form").validate({
                errorClass: "danger",
                successClass: 'success',
                errorPlacement: function (error, element) {
                    error.addClass('w-100').appendTo(element.parent('.form-group'));
            ...
            $("body").on('click','.datatable .edit',function (){
                ...
                $.ajax({
                    url:'{{route('admin.logistic.cn.child_issue_to_rider.edit',['id'=>':id']) }}'.replace(':id',id),
                    method:'GET'
                }).done(function (data){
                    if(data.status==0)
                    {
                        var trax_child_cn_issue_rider = data.trax_child_cn_issue_rider;
                        $("#id").val(trax_child_cn_issue_rider.id);
                        $("#edit_company_code").val(trax_child_cn_issue_rider.company_code);
                        $("#edit_cn_from").val(trax_child_cn_issue_rider.cn_from);
                        $("#edit_cn_to").val(trax_child_cn_issue_rider.cn_to);
                        $("#edit_rider_id_select").val(trax_child_cn_issue_rider.rider_id).trigger('change');

                        $("#EditChildCNIssueRiderModal").modal("show");

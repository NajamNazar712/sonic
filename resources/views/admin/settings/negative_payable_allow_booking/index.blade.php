...
@section('title', 'Negative Payable Allowed Shippers for Zero COD Bookings (48 Hours)')
    ...
    <div class="app-content content">
        <div class="content-wrapper">
            ...
            <div class="content-body">
                <h1 class="mb-1">
                    Negative Payable Allowed Shippers for Zero COD Bookings (48 Hours)
                </h1>
        ...
        <div class="modal fade text-left" id="AddStarShipper" data-backdrop="static" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    ...
                    <form method="post" id="shipper_add" action="{{route('admin.settings.zero_cod_shippers.add')}}">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group" id="select_shipper">
                                <select name="shipper_id" id="shipper_id" class="form-control select2"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#AddStarShipper').on('hidden.bs.modal', function () {
                $('#shipper_id').val('').trigger('change.select2');
            });

            var star_shipper_obj = [];
            star_shipper_obj.length = 0;

            $('#shipper_id').prepend('<option selected></option>').select2({
                width: '100%',
                placeholder: "Select Shipper",
                allowClear: true,
                dropdownParent:$('#shipper_add'),
            ...
            var table = $('#datatable').DataTable({
                ...
                ajax: '{{ route('admin.settings.zero_cod_shippers.list') }}',
                rowId: 'id',
                order: [[3, 'desc']],
                columns: [
                    {
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.remove', function () {
                var id = parseInt($(this).parents('tr').attr('id'));
                $.ajax({
                    url: '{!! route("admin.settings.zero_cod_shippers.remove") !!}',
                    method: 'POST',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'

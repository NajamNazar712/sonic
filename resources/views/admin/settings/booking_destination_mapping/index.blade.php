...
@section('title', 'Booking Destination Keyword')
        ...
        $(document).ready(function() {
            ...
            jQuery.fn.DataTable.Api.register( 'buttons.exportData()', function ( options ) {
                if ( this.context.length ) {
                    ...
                    params.excel = true;
                    var jsonResult = $.ajax({
                        url: '{{ route('admin.settings.booking_destination_keyword.list') }}',
                        data: params,
            ...
            var table = $('#datatable').DataTable({
                ...
                buttons:[
                        @if (session('role_id') == 1 || in_array(773, session('permissions')))

                    {
                        text: '<i class="la la-plus"></i> Add Keywords',
                        className: 'btn btn-primary add_area',
                        enabled: true,
                        action: function (e, dt, node, config) {
                            var redirect = '{!! url('/admin') !!}';

                            var url = redirect + '/settings/booking_destination_keyword/add';
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.enable_disable', function() {
                ...
                swal({
                    ...
                    $.ajax({
                        url:'{!! route("admin.settings.booking_destination_keyword.enable_disable") !!}',
                        method: 'POST',
                        data: {
                            'id': id,
                            '_token': '{{ csrf_token() }}'
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.edit', function() {
                ...
                var url = redirect + '/settings/booking_destination_keyword/edit/'+id;
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.view_keyword', function() {
                var redirect = '{!! url('/admin') !!}';
                var id = parseInt($(this).parents('tr').attr('id'));
                var url = redirect + '/settings/booking_destination_keyword/view/'+id;
                window.location = url;
            });

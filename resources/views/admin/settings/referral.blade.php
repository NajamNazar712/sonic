...
@section('content')
    <div class="app-content content">
        ...
        <div class="modal fade text-left" id="SubmitReferralModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="SubmitReferralModal"
         aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="">New Referral</h4>
                </div>
                <form method="post" id="submit_referral_form" action="{{route('admin.settings.referral.submit')}}">
                    @csrf

                <div class="modal-body">
                    
                    <div class="form-group">
                        <input type="text" name="referral_code" id="referral_code" class="form-control" placeholder="Referral Code*" data-rule-required="true" data-msg-required="Referral Code is required" data-rule-remote="{{ route('admin.settings.referral.name') }}" data-msg-remote="Referral Code must be unique">
                    </div>
                </div>
        ...
        $(document).ready(function() {
            ...
            var table = $('#datatable').DataTable({
                ...
                buttons:[
                    ...
                    
                    {
                        ...
                        className: 'btn btn-primary add_referral_code',
                        enabled: true,
                        action: function (e, dt, node, config) {
                            $('#SubmitReferralModal').modal('show');
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.enable_disable', function() {
                var id = parseInt($(this).parents('tr').attr('id'));
                                         $.ajax({
                                            url:'{!! route("admin.settings.referral.enable_disable") !!}',
                                            method: 'POST',
                                            data: {
                                                'id': id,
                                                '_token': '{{ csrf_token() }}'

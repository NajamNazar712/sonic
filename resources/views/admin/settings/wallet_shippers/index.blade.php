...
@section('content')
    <div class="app-content content">
        ...
        <div class="modal fade text-left" id="AddWalletShipper" data-backdrop="static" tabindex="-1" role="dialog"
             aria-labelledby="AddWalletShipper"
             aria-hidden="true">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="">Add Wallet Shippers</h4>
                    </div>
                    <form method="post" id="wallet_shipper_add" action="{{route('admin.settings.wallet_shippers.add')}}">
                        @csrf
                        ...
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-success" id="assign_wallet_shipper">Add</button>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#AddWalletShipper').on('hidden.bs.modal', function () {
                $('#wallet_shipper_id').val('').trigger('change.select2');
            });
            ...
            var table = $('#datatable').DataTable({
                ...
                ajax: '{{ route('admin.settings.wallet_shippers.list') }}',
                rowId: 'id',
                order: [[2, 'desc']],
                columns: [
                    {
                    ...
                    // {data: 'status', name: 'wallet_shipper_settings.status', class: 'align-middle status'},
                    {data: 'created', name: 'wallet_shipper_settings.created_at', class: 'align-middle created_at'},
                    // {
                    //     data: 'action',
                    //     name: 'action',
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.enable_disable', function () {
                var id = parseInt($(this).parents('tr').attr('id'));
                $.ajax({
                    url: '{!! route("admin.settings.wallet_shippers.enable_disable") !!}',
                    method: 'POST',
                    data: {
                        'id': id,
                        '_token': '{{ csrf_token() }}'

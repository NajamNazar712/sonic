...
@section('title', 'Lost Shipment Shippers')
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="">Add Shipper</h4>
                </div>
                <form method="post" id="add_shipper" action="{{route('admin.settings.lost_shipment_shippers.add')}}">
                    @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#shipper_id').prepend('<option selected></option>').select2({
                ...
                allowClear:true,
                dropdownParent:$('#add_shipper')
            });

            $('#edit_shipper_id').prepend('<option selected></option>').select2({
                width:'100%',
                placeholder:"Select Shipper",
                allowClear:true,
            ...
            var table = $('#datatable').DataTable({
                ...
                ajax: '{{ route('admin.settings.lost_shipment_shippers.list') }}',
                rowId: 'id',
                order: [[0, 'desc']],
                columns: [
            ...
            $('#datatable tbody').on('click', 'tr td.action .btn-group .dropdown-menu .dropdown-item.delete', function() {
                ...
                swal({
                        ...
                        dangerMode: true
                            ...
                            $.ajax({
                            url:'{!! route("admin.settings.lost_shipment_shippers.delete") !!}',
                            method: 'POST',
                            data: {
                                'id': id,
                                '_token': '{{ csrf_token() }}'

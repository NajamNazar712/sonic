                ...
                <div class="card">
                    <div class="card-content" aria-expanded="true">
                        <div class="card-body">
                            ...
                            <form id="arrival_of_shipments_form" class="form-horizontal text-center" method="POST" action="{{ route('admin.v2_pickups.arrival_service.service.store') }}" novalidate="novalidate">
                                {{ csrf_field() }}
            ...
            $('#add_shipment_form').validate({
                ...
                submitHandler: function(form) {
                    ...
                    if (table.columns('.tracking_number').data().eq(0).indexOf(parseInt(tracking_number)) === -1) {
                        $.ajax({
                            url: '{!! route('admin.v2_pickups.arrival_service.service.shipment_details') !!}',
                            method: 'POST',
                            data: {
                            ...
                            success: function(data) {
                                ...
                                if (data.status == 0) {
                                    ...
                                    if (index === -1) {
                                        ...
                                        table.row.add([rowNo + 1, data.details.tracking_number, data.details.shipper, data.details.amount, data.details.city ,data.details.hub ,remove_button]).node().id = data.details.id;
                                        table.draw(false);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.status == 0) {
                                ...
                                if (index === -1) {
                                    var rowNo = table.rows().count();
                                    var new_row = table.row.add([rowNo + 1, data.details.tracking_number, data.details.shipper, data.details.amount,data.details.city,data.details.hub, remove_button]).draw().node();

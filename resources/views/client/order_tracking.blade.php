                ...
                <div class="card">
                    <div class="card-content" aria-expanded="true">
                        <div class="card-body">
                            ...
                            <form id="track_form" class="form-inline mb-1 justify-content-center" novalidate="novalidate" autocomplete="off">
                                <div class="form-group">
                                    <input type="text" name="order_id" class="order_id" placeholder="Order ID*" data-tags-input-name="order_id" data-rule-required="true" data-msg-required="Order ID is required" >
                                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // var select = $('#track_form .order_id').selectize({
            ...
            @if (app('request')->has('order_id'))
            track({{ app('request')->input('order_id') }});
            @endif

            function track(order_id){
                $.ajax({
                    url: '{!! route('cod.order.track') !!}',
                    method: 'POST',
                    data: {
                        'order_id': order_id,
                        '_token': '{{ csrf_token() }}'
                    ...
                    .done(function(data) {
                        ...
                        if (data.shipments != undefined) {
                            $.each(data.shipments, function(id, details) {
                                ...
                                shipment += '<div class="mb-0 ml-1 font-medium-3 white">' + details.tracking_number + ' - Order ID: '+ details.order_id + '</div>';

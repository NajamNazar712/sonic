...
(CASE WHEN (`sj`.`shipper_status_id` = 2) THEN 1 ELSE 0 END) AS `arrival_count`,
'Arrival'                AS `Arrival`,
NULL                     AS `manifest_code`,
...
NULL                      AS `NULL`,
NULL                      AS `arrival`,
(CASE WHEN (`sj`.`shipper_status_id` = 3) THEN 1 ELSE 0 END) AS `manifs_cont`,
'manifest'                AS `manifest`,
NULL                      AS `Misroute_code`,
'Misroute'                AS `Misroute`,
NULL                      AS `wm_code`,
'Without_manifest'        AS `Without_manifest`
FROM (((((((((`shipments` `sh`
...
NULL                       AS `NULL`,
NULL                       AS `NULL`,
(CASE WHEN (`sj`.`shipper_status_id` IN (68,11)) THEN 1 ELSE 0 END) AS `mr_cont`,
'Misroute'                 AS `Misroute`,
NULL                       AS `wm_code`,
'Without_manifest'         AS `Without_manifest`
FROM (((((((((`shipments` `sh`
...
$manifest_report2_excel = "CREATE OR REPLACE VIEW `manifest_report2_excel` AS
SELECT
  CAST(`k`.`created_at` AS DATE)       AS `booking_date`,
  `k`.`origin_zonecode`                AS `origin_zonecode`,
  `k`.`destination_zonecode`           AS `destination_zonecode`,
  `k`.`shipment_receiving_destination` AS `receiving_city`,
  `k`.`shipment_tracking_number`       AS `tracking_number`,
  `k`.`parent_prod_name`               AS `parent_prod_name`,
  `k`.`sub_prod_name`                  AS `sub_prod_name`,
  ...
  (CASE 
	...
	WHEN (`k`.`shipper_status_id` = 67) THEN 'Without Manifest Shipment'
  ELSE 0 END) AS `Remarks`
FROM `manifest_report` `k`
WHERE shipper_status_id IN (11,67,68)
";

DB::statement($manifest_report);
DB::statement($manifest_report2);
DB::statement($manifest_report2_excel);
}
...
DB::statement("DROP VIEW IF EXISTS manifest_report2_excel;");
}
}

...
class UpdateMissingShipmentsSeeder extends Seeder
{
    ...
    public function run()
    {
        $tracking_numbers = [29314428455898, 20217228455686, 20217228455720, 20217228455783, 20222328456577, 20222328456600, 20211028456622, 22317428456812, 22317428456832, 22320228456717, 22320228456897, 22320228456903, 22320228456906, 20225128455335, 20213428455544, 31120228456420, 31124728456449, 22331528456512, 22310728456514, 22317428456773, 22330228456786, 22322328456813, 22320228456827, 22314428456829, 22315828456855, 22326928456942, 22322328456986, 22313428456951, 14422328456926, 20215928456403, 20233328456948, 25125128455528, 28320628456788, 28320628456818, 20220228456505, 20220228456524, 22320228456933, 33920228456417];
        ...
        foreach ($tracking_numbers as $tracking_number){
            ...
            if($shipment){
                ...
                if(Shipment::where('tracking_number', $tracking_number)->exists()){
                    continue;
                }
                ...
                if ($status_code == 200) {
                    ...
                    Log::channel('trax_pay_test')->info($tracking_number . ' : ' .json_encode($payload_booking, true));
                    if ($payload_booking->status == 0) {
                        $new_tracking_number = $payload_booking->tracking_number;
                        Log::channel('trax_pay_test')->info('new tracking: '. $new_tracking_number . 'old tracking' . $tracking_number);
                        $sonic_shipment = Shipment::where('tracking_number', $new_tracking_number)->first();

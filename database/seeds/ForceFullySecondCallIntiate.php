...
class ForceFullySecondCallIntiate extends Seeder
{
    ...
    public function run()
    {
        ...
        if (count($shipmentSeconds) > 0) {
            Log::channel('botCallJobLog')->info('s ' . 'Call initiate forcefully start second-call' . Carbon::parse(now())->format('Y-m-d H:i:s'));
            foreach ($shipmentSeconds as $shipmentId) {
                dispatch(new BotCallDispatchSecod($shipmentId));
            }
            Log::channel('botCallJobLog')->info('s ' . 'Call initiate forcefully end second-call' . Carbon::parse(now())->format('Y-m-d H:i:s'));
        }
    }
}
